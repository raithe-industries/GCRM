...
- **Geography** — feeds are Canada/US-dense; SW-Pacific seeded via `geonet_volcano` (NZ),
  ...
  **EXTENDED 2026-07-11** with `wa_dfes` (**WA DFES EmergencyWA all-hazard warnings** — Western Australia,
  ...
  feed, covering the cyclone-prone Pilbara. Remaining Australia hunts: the other state services (**Qld
  bushfire, Tasmania Fire Service, EMV Victoria** — same GeoRSS/GeoJSON family, committed HA fixtures exist),
  **Geoscience Australia** hazards, **BoM** (when it resumes open data). **Europe SEEDED 2026-07-08** with `ea_flood` (**UK Environment Agency
