...
- **Geography** — feeds are Canada/US-dense; SW-Pacific seeded via `geonet_volcano` (NZ),
  ...
  WebFetch, can't confirm read-auth). Mapping EMMA region codes → polygons needs a region
...
- **Domains under-covered** — power-grid stress (other ISOs), rail/pipeline incidents
  ...
  **EAWS / SLF investigated 2026-06-28 — blocked, deferred:** authoritative + auth-free + a
  ...
  in WebFetch, and the GitHub geometry/example repos (`eaws/eaws-regions`, `caaml/caaml`,
  `simon04/eaws-bulletin-map`) **can't be enumerated** (GitHub MCP scoped to `raithe-industries/gcrm`;
  `github.com` 403s; `raw.githubusercontent.com` serves only known paths) and WebSearch surfaced
...
- **2026-06-30** — **adopted `bmkg_quake` (BMKG / InaTEWS Indonesia felt earthquakes), Path A** — a new
  ...
  geometry-bearing GeoJSON is the registration-gated `api.meteoalarm.org` (403 in WebFetch — can't distinguish
  ...
  (`gempadirasakan.json`), and `feeds.meteoalarm.org`-via-page all **403 via WebFetch** (egress-wide non-GitHub
  ...
  (WebFetched its README off GitHub-raw — confirms the three open products `autogempa`/`gempaterkini`/
...
- **2026-06-28** — honest **NO-OP** after a wide hunt. Per the SUSTAINED-BLOCK DIRECTIVE I LED with a
  ...
  *quality* bars on paper — authoritative (SLF = Swiss federal WSL/SLF; EAWS national services), auth-free,
  ...
  verification, and is refused. **Network re-probed fresh:** WebFetch positive control on
  ...
  (egress-wide WebFetch block on non-GitHub hosts unchanged — only `raw.githubusercontent.com` resolves).
  **Why no real-bytes anchor (the difference from the last 7 successful runs):** NHC→JMA→GeoNet→USGS-volcano→
  ...
  examples); WebSearch surfaced **no** concrete indexed sample/geometry file URL. The CAAML v6 field shape was
...
- **2026-06-27** — **adopted `magma_volcano` (PVMBG / MAGMA Indonesia volcano alert levels), Path B** —
  ...
  (egress-wide WebFetch block on non-GitHub hosts unchanged). Volcano targets named in the ledger ruled
  ...
  NHC→JMA→GeoNet→USGS-volcano→NWPS):** confirmed via **WebSearch** that PVMBG/MAGMA is auth-free, then
...
- **2026-06-26** — **adopted `usgs_volcano` (USGS HANS US/Alaska Volcanic Alert Levels)** — a new
  ...
  **WebFetch positive control** on `raw.githubusercontent.com` correct (`facebook/react`
  `package.json` → `private:true`/no `name`); the egress-wide WebFetch 403 on non-GitHub hosts is
  unchanged (so the live HANS origin still can't be hit in-sandbox; prod's full network fetches it).
  **The unlock:** confirmed via **WebSearch** that HANS `getElevatedVolcanoes` is open/no-auth/JSON,
