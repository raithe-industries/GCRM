    ...
    fn fetch_next(&mut self) -> XmlResult<Option<XmlEvent>> {
        ...
        loop {
            ...
            match event {
                ...
                // VENDOR PATCH (GCRM): quick-xml ≥0.36 emits entity references as
                // separate GeneralRef events instead of resolving them inside Text.
                // Unhandled, they were silently DROPPED here — "&lt;p&gt;x" arrived
                // downstream as "px", destroying encoded markup in RSS descriptions.
                // Resolve the predefined + numeric entities to their text.
                Event::GeneralRef(ref e) => {
                    let name = reader.decoder().decode(e.as_ref())?.to_string();
                    let resolved: Option<String> = match name.as_str() {
                        "lt" => Some("<".into()),
                        "gt" => Some(">".into()),
                        "amp" => Some("&".into()),
                        "apos" => Some("'".into()),
                        ...
                        n if n.starts_with("#x") || n.starts_with("#X") => {
                            u32::from_str_radix(&n[2..], 16).ok().and_then(char::from_u32).map(String::from)
                        }
                        n if n.starts_with('#') => {
                            n[1..].parse::<u32>().ok().and_then(char::from_u32).map(String::from)
                        }
                        _ => None, // unknown custom entity — keep prior (drop) behavior
                    };
