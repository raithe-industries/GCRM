...
already-surfaced state, not a relocation/restyle. **The blind/thin/stale/capped/held caveat family is
CLOSED — no further T2 credit from it.**

**T3 — annotation of an already-visible observable.** Mirror a caveat to an Nth surface, restyle,
relabel, methodology prose for something already shown. The per-modality I&W light family (13 lights,
5/5 modalities, complete) and the blind/thin/stale/capped/held caveat family are **CLOSED: mining
them is FORBIDDEN, not even T3.** Genuine T3 polish is allowed ONLY when no T1/T2 can be done well
...
- **Closed veins are FORBIDDEN, not last-resort:** the per-modality I&W light family and the
  blind/thin/stale/capped/held caveat family yield NO credit at any tier. A run that mines them FAILS.
...
- **Annotation inflation** — mirroring an existing caveat/flag/observable onto an Nth surface, or
  adding the Nth per-modality I&W light. The light family (13 lights, 5/5 modalities) and the
  blind/thin/stale/capped/held caveat family are DECLARED CLOSED: mining them is forbidden.
...
## Metrics (baselines as of 2026-06-09; test-count baseline 2026-06-25)
...
| Test count | locks behavior — **NEVER evidence of improvement** | `grep -rhoE '#\[(tokio::)?test\]' src \| wc -l` | **463** | **Floor (Hold)** — a run whose ONLY positive effect is +N tests is an automatic FAIL; the required lock test must FAIL when the change is `git stash`ed (attach that proof) |
...
| Calibration evidence | the number is earned | `cargo test calibration_evidence_report -- --nocapture` | **Brier ~2e-6 / RMSE 0.14pp / in-band 4/4** | **↓ Brier** (lower = better-fit; never regress upward without a documented live-targeted reason) |
...
| Feed liveness | every source live or replaced | `cargo test --release feed_roster_liveness -- --ignored --nocapture` (local only) | **102/103** (2026-06-09) | **↑** / never ↓ |
