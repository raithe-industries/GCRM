...
## 2026-07-24 (later²) — robustness (DISPLAY/thumbnails) — the capture worker no longer lets a heavy page swarm the shared box: YouTube fetches its poster (no browser), a process-count circuit breaker caps any page at ~40 procs, and duplicate/overlapping captures are de-duped
- Item: ad-hoc (follow-up to the same-day thumbnail work — verification showed a single capture spiking chromium to ~88 procs on the shared prod box).
- Diagnosis: with hydration live, loading the dashboard queues every knock's capture. Two pages misbehaved: (a) YouTube watch/LIVE pages spawn a huge media-decode tree (~88 procs) and usually render a consent wall; (b) an ad/tracker-heavy article (independent.co.uk, measured 85 procs) swarms UTILITY/network subprocesses that `--renderer-process-limit` can't cap, and times out with nothing. cgroup `pids.max` is not a fit — it counts THREADS, so any limit that lets chromium run also lets it explode (TasksMax=28 SIGKILLed it). `--single-process` SIGKILLs modern headless. Aggressive media flags made Google News time out.
- Change (all in `thumbs.rs`, capture path only — no P/engine/display touch): (1) VIDEOS never touch the browser — `youtube_id()` parses the id from every url shape and `youtube_poster()` fetches `img.youtube.com/vi/<id>/{maxresdefault→mqdefault}.jpg` (16:9, always-present fallback) via reqwest, resized to the same JPEG. A truer preview than a consent wall AND kills the biggest process source. (2) PROCESS-COUNT CIRCUIT BREAKER: `capture_one` races the wait against a 1 Hz `count_process_group()` poll (`/proc/<pid>/stat` field 5); a group past `MAX_CAPTURE_PROCS=40` is pathological (it won't yield a good shot) → SIGKILL the group early instead of riding 85 procs to the timeout. (3) `--renderer-process-limit=1` + no site-isolation caps renderer fan-out (screenshot byte-identical to unconstrained). (4) IN-FLIGHT DEDUP: an `INFLIGHT` key-set so the same url queued by N concurrent bootstraps is captured once; (5) a 750 ms inter-capture settle so a finished browser's teardown doesn't stack on the next spawn.
- Metric moved: peak chromium during a full-timeline capture pass fell **88 → 47 procs** (capped near 40 + sub-second overshoot), YouTube cards now show real video frames (France 24 "Eye on Africa" poster visually confirmed), and the worker still drains to 0 with no leak. +2 lock tests (`youtube_ids_parse_from_every_url_shape`, `inflight_claim_dedupes_then_releases`), 663 → 665 passed.
- Proof: `cargo build --release` green (1m30s). `cargo clippy --release` — 0 warnings. `cargo test --release` → **665 passed / 0 failed / 5 ignored**. Verified on :8199 (staged prod logs): 3 concurrent bootstraps, peak chromium 47 (was 88), 7/8 YouTube posters fetched + a real article screenshot, chrome drained to 0; eyes gate green. Flag/limit choices were measured, not guessed (documented rejections: single-process, cgroup TasksMax, aggressive media flags).
- Tier: T2 · Touched: display-only (capture worker; no P math, no fitted constant, no anchor, no I&W light) · Lock-fails-without-change: yes (`youtube_ids_parse_...` pins the id parser the video path depends on; `inflight_claim_...` pins the dedup) · Counts: none of Live-sources/Map-layers/Monitors moved · consecutive_display_only=3 · display_only_in_last_7=5 · consecutive_noop=0 · noop_in_last_3=0
- Notes / decisions future runs must respect: (1) The capture path has THREE independent process ceilings — group-kill on timeout, the process-COUNT breaker (`MAX_CAPTURE_PROCS`), and `--renderer-process-limit=1`; do not remove any, they cover different failure modes (a slow page, a swarming page, a fan-out page). (2) YouTube (and only YouTube, for now) is poster-fetched — never route it back through the browser; other video hosts fall through to the bounded browser path. (3) `--single-process` and cgroup `pids.max`/systemd `TasksMax` are KNOWN-BAD here (SIGKILL / thread-counting) — don't reach for them. (4) `count_process_group` scans `/proc` at 1 Hz during ONE serialized capture — fine; do not call it per-tick or from a hot loop.

## 2026-07-24 (later) — fix (DISPLAY/thumbnails) — the mini-pageview cards showed NO image: historical knocks carried no thumb key and captures only fired on rare live knocks — now every served knock is hydrated with a key + queued capture, and the card image retries until the shot lands
- Item: ad-hoc (operator report: "I am not seeing any images on the cards; pinning works but no graphical pageview").
- Diagnosis: the feature shipped 2026-07-24 minted a thumb key ONLY on FRESH live knocks (the aggregator's tick-driver-ref builder). But almost every hoverable knock is SEEDED FROM DURABLE HISTORY (the ring / WS-bootstrap / `/api/epoch`), and those refs predate the feature so their `thumb` field is empty → the client rendered no `<img>` at all. Compounding it, captures only enqueue on a live knock, and the market was quiet post-deploy, so `thumbs/` was empty (0 files) and even a fresh key would 404. Net: cards were imageless in exactly the common case. Verified on prod: `/api/epoch` had 13 driver refs, 12 with urls, **0 with thumb keys**; `thumbs/` = 0 files.
- Change (display + new `thumbs::hydrate_timeline`, still no P/engine touch): (1) both timeline-serving paths (WS bootstrap + decimated `/api/epoch`) now run `hydrate_timeline(&mut entries)` — for every driver ref with an http(s) url it mints `thumb = key_for(url)` in the served JSON (so the `<img>` renders) and enqueues a capture NEWEST-FIRST, budget-bounded (48 < queue depth 64) so recent knocks capture before the queue fills and repeated bootstraps are cheap no-ops once a shot is on disk. (2) Client: the card `<img>` no longer hides on the first 404 — `tlShotErr` RETRIES with a cache-busting param (~8×2.6s ≈ 20s, covering the measured 1–28s capture span), then drops the frame; a pulsing "◵ capturing preview…" placeholder shows meanwhile and the loaded JPEG paints over it. The pinned card (which persists) is what lets the retry play out and the pageview pop in.
- Metric moved: the imageless-card regression is closed — a hovered/pinned knock now shows the actual source page. Verified end-to-end on a throwaway :8199 (staged prod logs): `/api/epoch` refs went 0→12 with thumb keys; the worker captured 41 pages (a real Independent article pageview visually confirmed) then drained chrome to 0; a headless DOM check pinned a hydrated historical knock and the card `<img>` **actually loaded — `naturalWidth=384`** — with the placeholder present and zero pageerrors. +2 lock tests (`hydrate_backfills_keys_for_historical_refs_with_urls` + the updated card-contract leg), 662 → 663 passed.
- Tier: T2 · Touched: display-only (+ serve-time hydration; no P math, no fitted constant, no anchor, no I&W light) · Lock-fails-without-change: yes (`hydrate_backfills_keys_...` asserts a historical http ref gains its `key_for(url)` while non-http/keyed refs are untouched; the card leg asserts `onerror="tlShotErr(this)"` + `function tlShotErr`) · Counts: none of Live-sources/Map-layers/Monitors moved · consecutive_display_only=2 · display_only_in_last_7=4 · consecutive_noop=0 · noop_in_last_3=0
- Notes / decisions future runs must respect: (1) The capture-leak guard from the parent entry STILL HOLDS under the higher serve-driven volume — verified one process-GROUP at a time (serialized worker), transient teardown zombies reaped, chrome drains to 0 when the queue empties; the brief 62-proc peak was one heavy YouTube page's helper tree overlapping a teardown, NOT a serialization break. (2) hydrate is idempotent + budget-bounded on PURPOSE — do not remove the 48-cap or the newest-first `rev()`, or a deep bootstrap would flood the 64-slot queue with ancient knocks and starve the recent ones the operator is actually looking at. (3) The retry lives CLIENT-side against the existing `/api/thumb/:key`; there is deliberately NO client→server "capture this url" endpoint (that would be an SSRF vector — the server only ever captures urls IT put in the served timeline). (4) YouTube watch-page captures are heavy + often consent-walled; `is_boilerplate_render` drops identical walls, and a wall/timeout just leaves that one card text-only.

## 2026-07-24 — legibility/awareness (DISPLAY + new module) — the timeline knock card now leads with a MINI PAGEVIEW (a captured screenshot of the driver's source page) and a click on the chart PINS the card so its "open ↗" link stops disappearing before you can hit it
- Item: ad-hoc (operator directive, Robert-interactive) — "screenshot the article/video source page, save a tiny version that is eventually overridden to avoid overusing drive space, present a minimized mini-pageview in the card that links to the source; make the timeline graph more easily clickable because the correct 'open' disappears too quickly."
- Change (new module `src/thumbs.rs` + display; NO engine/P/anchor touch): (1) A single background worker consumes a bounded (64) queue of source URLs off the aggregator's driver-ref builder and captures each with the headless Playwright chromium already on the box (`nice -n 15`, `--headless --screenshot --virtual-time-budget=7000`), downscales to a 384px-wide JPEG (q72, ~15-25KB), and stores it under gitignored `thumbs/` keyed by `md5(url)[..20]`. Disk is bounded by a rolling cap (300 files, oldest-by-mtime evicted) — the "eventually overridden" the operator asked for (~6MB ceiling). `DriverRef` gains a `thumb` key (skip-default), served at `GET /api/thumb/:key` (hex-only key = path-traversal guard; 7-day immutable cache; JPEG bypasses the gzip layer). The card renders it as a top-cropped `<img loading=lazy>` that removes its own frame on 404 (a capture legitimately lags its driver by seconds) so a not-yet-captured page shows text-only, never an empty box. (2) Click-to-pin: a click anywhere on the chart freezes the current hover card (`_tlPin`) — it ignores further hover, never auto-hides, and is dismissed by its ✕ / Escape / click-away; the unpinned hover grace also went 250ms → 600ms. This is the fix for "tricky to click the correct open because it disappears too quickly" and doubles as the touch path (tap = hover+click → render+pin).
- Metric moved: every timeline knock's driver card is now a recognisable pageview the operator can click through without racing a vanishing tooltip. +2 lock tests (thumbs wall-dedup + the card's 5th-leg contract legs folded into the existing knock test); 660 → 662 passed. New sight added (the pageview surface), not a new engine light — board stays at 12, no caveat-family change, P/index/anchors bit-identical.
- Proof: `cargo build --release` green (1m28s). `cargo clippy --release` — 0 warnings. `cargo test --release` → **662 passed / 0 failed / 5 ignored**. Verified end-to-end on a throwaway :8199 instance against staged prod logs: captures render as clean pageviews (visually confirmed Defense News / Moscow Times / Taipei Times headline+byline+body), served 200 `image/jpeg` immutable; eyes gate GREEN; a headless-Playwright DOM check confirmed the card shows the `/api/thumb/<key>` img, renders 1 linked + 1 unlinked row, pins on click (✕ + "Esc to close"), survives hover-away while pinned, closes on Escape, hides the shot frame on a 404 thumb, and throws ZERO pageerrors.
- Tier: T2 · Touched: display-only (+ new off-path capture module; no P math, no fitted constant, no anchor, no I&W light — `git stash` of the change fails the two new locks but moves no calibration band) · Lock-fails-without-change: yes (the card-contract legs assert `/api/thumb/`, the key re-validation regex, the 404 onerror frame-hide, `onClick:()=>pinTlCard()`+`if(_tlPin)return;`, and the ✕/Escape dismissal; `thumbs::identical_renders_...` pins the wall-dedup) · Counts: none of Live-sources/Map-layers/Monitors moved (a display sight, not a data feed) · consecutive_display_only=1 · display_only_in_last_7=3 (2026-07-22 command-strip + 2026-07-17-later 2.10 + this) · consecutive_noop=0 · noop_in_last_3=0
- Notes / decisions future runs must respect: (1) The capture path is OFF the request path by construction — one serialized worker, bounded queue (drop when full), rolling disk cap; NEVER move capture onto a request handler or make `/api/thumb` synthesize on miss. (2) The timeout MUST kill the browser's whole PROCESS GROUP (`cmd.process_group(0)` + `kill -KILL -pid`): a plain `timeout(child.wait())` merely drops the future and the chromium keeps running — during verification that leaked 135 orphaned chromium in minutes. Do not "simplify" the group-kill away. (3) Measured real pages span 1–28s to first frame (aljazeera.com/news is the slow end) — the 40s `CAPTURE_TIMEOUT_SECS` is deliberate; a tighter bound discards good captures. (4) Cookie-wall / paywall / error pages render byte-identically across URLs; `is_boilerplate_render` drops any render whose md5 already exists under a DIFFERENT key (seeded from disk on boot) — keep it, it's why the card shows articles not consent banners. (5) `GCRM_THUMBS=0` is the kill switch; no chromium ⇒ feature stays dormant and the card falls back to text rows exactly as before. (6) The tested capture browser is Playwright's cached chromium (`~/.cache/ms-playwright/...`), the same one the eyes gate uses; `GCRM_CHROME` overrides.

---

## 2026-07-23 — honesty (ENGINE/economic-warfare) — `chokepoint_pair_score` now matches coercion tokens by WORD BOUNDARY, not bare substring — the one keyword path in processor.rs that never got the substring→boundary fix, so ordinary shipping-desk prose can no longer fabricate "chokepoint weaponized" (→ `economic_warfare` → P AND the `energy_chokepoint` I&W light)
- Item: roadmap 1.ag (new; surfaced by a fresh honesty bug-hunt of the less-mined engine seams — models/processor/indicators/brief/video).
- Diagnosis (weakest reachable seam): suite green (653 passed / 5 ignored — no failing/flaky test to fix first). The previous run (2026-07-22) was display-only AT the 2-of-7 cap, so this run HAD to be substantive (T1/T2) or a structured NO-OP. New sources/map feeds are the signal-hunter's lane; monitors/§8 need live connectors; fitted constants + the open candidates 1.ad/1.ae are Robert-gated; 1.x needs a week of labeled pairs. A fanned-out bug-hunt of the seams NOT recently mined surfaced one genuine, ironclad, P-feeding HONESTY defect: `chokepoint_pair_score` (processor.rs:~1225) is the SOLE keyword path in the file still using bare `tl.contains(...)` — the sibling `score_domains` right below it word-boundary-matches every short token (`WORD_START_DOMAIN_KWS`/`BOUNDARY_DOMAIN_KWS`), and 1.7/1.8/1.29/1.30 applied the same fix to actors/domains. Its coercion tokens are tiny (`mined`/`mining`/`shut`/`fee`/`toll`/`seize`/…), so they hit mid-token in ordinary words. The pair rule noisy-OR-merges 0.80 into `economic_warfare`, which flows to theater heat → the systemic index / P AND independently trips the `energy_chokepoint` board light — a served number biased UP and an operator indicator flipped, the exact false-alarm direction the file's boundary work exists to prevent.
- Change (engine-behavior; no P math, no fitted constant, no anchor touched): coercion tokens (`HARD_COERCION` + `FEE_COERCION`) now match at a WORD START (`starts_word` — keeps plural/tense forms like `tolls`/`seizes`/`shutdown` while killing mid-token hits: `"determined"`⊃`mined`, `"examining"`⊃`mining`, `"atoll"`⊃`toll`, `"return back"`⊃`turn back`), EXCEPT the two tokens whose very PREFIX is itself an ordinary word (`shut`⊂shuttle/shutter, `fee`⊂feel/feet/feed) which match WHOLE-word with their wanted forms enumerated (`shuts`/`shutdown`/`shutting`, `fees`). The strait/tolled PLACE names stay substring — they are long specific phrases with no realistic mid-token collision and are not the defect. The 0.80 weight and the merge are unchanged.
- Metric moved: benign trade/shipping journalism naming a real chokepoint (e.g. "regulators have not determined whether red sea shipping costs will rise", "coffee shipments through the strait of hormuz slowed", "ferry shuttle across the taiwan strait") can no longer return `Some(0.80)` economic_warfare or light `energy_chokepoint` off a mid-token substring — a served/P-feeding false-positive is closed. +1 lock test (653 → 654 passed). Never retuned a constant; the four anchors (quiet/Ukraine/current 60/Cuba) are bit-identical (backtest titles carry no chokepoint prose; all bands in-band).
- Proof: `cargo build --release` green (1m50s). `cargo clippy --release -p gcrm` — 0 warnings. `cargo test --release` → **654 passed / 0 failed / 5 ignored** (`bands_quiet/ukraine/current_full/cuba` all ok; `chokepoint_weaponization_phrasing_scores_economic_warfare` + both `indicators::chokepoint_light_*` still green — every prior true-positive preserved). Lock `chokepoint_coercion_tokens_match_words_not_substrings` proven fails-without-change EMPIRICALLY: reverting the `hit` closure to `tl.contains(token)` makes the "determined"⊃"mined" false case FAIL (`Some(0.8)`, panic at processor.rs:1735); restored → 654 green.
- Tier: T1 (engine-behavior — corrects a REACHABLE, P-feeding, high-frequency false-positive on a served path so ordinary shipping-desk prose can no longer fabricate a weaponized-chokepoint read; it changes WHETHER the pair rule fires and thus the `economic_warfare` signal reaching P and the `energy_chokepoint` light, locked by a fails-when-reverted test with a written contract rationale — the file's own substring→boundary discipline that every sibling path already carries; not a new light/annotation/surface, the board stays at 12, not in the closed blind/thin/stale/… caveat family. Same class as 1.7/1.8/1.29/1.30/1.32 — the substring→word-boundary honesty lineage. Candid scope note: it is an existing served path, not a new sight, and the effect surfaces only when a real chokepoint name co-occurs with an ordinary word carrying a coercion substring — but that co-occurrence is COMMON in maritime/trade copy (unlike the seasonal/contrived reachability of some recent fixes), so this is a high-frequency correction. I claim T1 because it is behavior-changing on a served P-feeding path with a stashable lock, matching the tier's engine-behavior gate.) · Touched: engine-behavior · Lock-fails-without-change: yes (revert `hit`→`tl.contains` → "determined"⊃"mined" false case panics at processor.rs:1735) · Counts: none of Live-sources/Map-layers/Monitors moved (a correctness fix, not a new sight) · consecutive_display_only=0 (engine-behavior, resets the streak from the 2026-07-22 display-only run) · display_only_in_last_7=2 (the 2026-07-22 command-strip + 2026-07-17-later 2.10 still in trailing-7; this run is engine-behavior, adds none) · consecutive_noop=0 · noop_in_last_3=0
- Notes / decisions future runs must respect: (1) The pair rule's coercion tokens match at a WORD START; the two prefix-colliding tokens (`shut`, `fee`) match WHOLE-word — do NOT "simplify" any coercion token back to `tl.contains(...)` (that re-admits the shuttle/coffee/atoll false fires; the lock pins the five mid-token cases → `None`). (2) When adding a coercion token, decide its matcher: word-start unless its PREFIX is a common word (then whole-word + enumerate the wanted inflections), mirroring `WORD_START_DOMAIN_KWS` vs `BOUNDARY_DOMAIN_KWS`. (3) The strait/tolled PLACE names intentionally stay substring (long specific phrases). (4) This is a served/P-feeding honesty fix; the 0.80 weight was NOT retuned and the four anchors are untouched. (5) The bug-hunt also flagged a MODERATE lead: `extract_casualties` (processor.rs:~1044) counts non-casualty person-nouns (`people`/`civilians` used for evacuations/displacement) into `severity` → intensity/P — the `troops`→size behavior is deliberately locked, so any fix must scope to the evacuation/protest over-read only; filed for a future run to verify against current code before acting.

---

## 2026-07-22 — legibility (EYES-GATE / command strip) — the deploy-time eyes gate now SEES the top-of-cockpit command strip: it verifies Threat/Risk/Confidence/Momentum populate off the `—` placeholder (with format checks on Risk% + Momentum), and a Rust anti-drift test locks all six cell IDs against a refactor drop
- Item: roadmap 2.11 (new; Lane-2 VISUAL-ANALYTIC — the eyes-gate-coverage lineage of 2.7/2.9/2.10).
- Diagnosis (weakest reachable pillar): suite green (652 passed / 5 ignored — no failing/flaky test to fix first). HONESTY's reachable served-path bug-hunt vein is heavily mined (1.z/1.aa/1.ab/1.ac/1.35/1.36/1.af all closed; the remaining open candidates 1.ad/1.ae are Robert-gated design/calibration judgments, 1.x needs a week of labeled pairs); new sources + map feeds are the signal-hunter's lane; monitors/§8 need live connectors the sandbox can't reach; fitted constants are Robert-gated. The weakest reachable pillar is LEGIBILITY-verification: the eyes gate is exhaustive on the awareness footers, context strip and the 12-cell I&W board, yet it verified only ONE of the SIX command-strip cells (`#cmd-trend`) — the top "grasp at a glance" row (Threat / Risk / Driver / Confidence / Momentum), the first thing an operator reads, could go entirely blind (`—` across every cell) and still pass the gate. Extending the gate is the least-recently-advanced lane and closes a genuine blind spot on the most-glanced surface.
- Change (verification/robustness; NO engine behavior, NO served value, NO dashboard.html, P untouched): (a) `deploy/eyes/smoke.mjs` §8b — a new section that polls the DOM (WS-race-safe, same 7.5s poll as §8) and asserts #cmd-threat / #cmd-risk / #cmd-conf / #cmd-mom populate off the `—` placeholder, with well-formed-format checks on the two stable formats (Risk `^≥?\d+(\.\d+)?%$`, Momentum `^([⇧⇩] )?[+-]?\d+\.\d{2}$`) so a stray/malformed token also fails; #cmd-driver is honest-null-capable (no dominant domain → `—`, its sub-line carries the fallback) so the node must EXIST but the placeholder is tolerated — "floor, not cage." (b) `src/server.rs` — `dashboard_command_strip_cells_render_from_the_snapshot`: locks all six cell IDs present in the served HTML AND each wired to a `getElementById(...)` repaint (a dropped element OR a dropped render both fail `cargo test` in-sandbox; previously only #cmd-trend and #cmd-mom carried individual ID locks). The eyes-gate half promotes STAGED→DONE at the local deploy that runs the browser gate.
- Metric moved: eyes-gate coverage of the command strip 1/6 cells → 6/6 (verified at runtime by §8b, structurally locked in-sandbox by the new Rust test); a top-row blind-render regression that previously shipped gate-green is now caught. Test count 652 → 653 (+1 lock). Build + full suite green; four calibration anchors untouched (no P/engine path touched).
- Proof: `cargo build --release` green; `cargo test --release` → 653 passed / 0 failed / 5 ignored. New Rust lock proven to FAIL when a cell id is dropped (renamed `id="cmd-conf"`→`cmd-CONFX` in a throwaway edit → test FAILED with "command strip dropped the #cmd-conf value cell"; dashboard.html restored, `git diff --stat` clean). `node --check deploy/eyes/smoke.mjs` OK; the Risk% and Momentum regexes validated against the render's actual output forms (`⇧ +0.42`/`⇩ -0.31`/`+0.04`/`-0.10` all match; `≥89.00%`/`60.12%` match).
- Tier: T3 (LEGIBILITY-verification/robustness on an already-visible surface — the command strip; it strengthens the system's OWN eyes over the most-glanced row and locks the contract in-sandbox, but adds no new sight/compute (not T1) and surfaces no first-time honesty state (not T2), so it is honestly T3, taken under the scorecard's "genuine T3 polish is allowed ONLY when no T1/T2 can be done well today AND neither display-only cap is breached." It is NOT one of the two CLOSED veins — it touches neither the I&W light board (stays at 12) nor the blind/thin/stale/… caveat family; it is the eyes-gate-coverage lane the scorecard explicitly keeps open ("extending it is how the system learns to SEE its weaknesses"), the same lineage as 2.7/2.9/2.10. Candid scope note: no T1 was reachable by me today — new sources/map feeds are the signal-hunter's lane, monitors need live connectors, fitted constants + 1.ad/1.ae are Robert-gated, and the reachable served-path defect vein is mined out; so I closed the single most valuable UNVERIFIED-surface gap in my lane and proved it with a fails-when-dropped in-sandbox lock.) · Touched: display-only (no engine behavior / no served value / no P; a verification+lock run) · Lock-fails-without-change: yes (the Rust lock FAILS when a cmd cell id is dropped — proven above; the eyes-gate §8b half is not sandbox-runnable and promotes at the local deploy, as the scorecard's two-phase rule prescribes for Eyes) · Counts: none of Live-sources/Map-layers/Monitors moved (a verification/robustness run, not a new sight); eyes-gate command-strip coverage 1/6 → 6/6 · consecutive_display_only=1 (this run is display-only; the 2026-07-19 read_range run was engine-behavior=0) · display_only_in_last_7=2 (this run + the 2026-07-17-later 2.10; the 2026-07-16 1.y aged out) — AT the 2-of-7 cap, not over; the NEXT run must be T1/T2 or a structured NO-OP, never a 3rd display-only · consecutive_noop=0 · noop_in_last_3=0
- Notes / decisions future runs must respect: the command strip is now locked at SIX cells end-to-end (Rust ID/repaint lock + eyes §8b runtime check) — a deliberate cell change is operator-gated anyway (six-balance, 2026-07-18) and must update BOTH locks. §8b tolerates #cmd-driver's honest-null `—` (no dominant domain) BY DESIGN — do not "tighten" it to demand a value, that would false-fail a diffuse world. The two format regexes track the operator-fixed 2026-07-18 momentum render (bare signed value when neutral, ⇧/⇩ when decisive) and the ceiling-prefixed risk (`≥` at cap) — update them in lockstep if that render changes. The streak is AT the display-only cap: the next routine run MUST be substantive (T1/T2 engine-behavior/new-sight) or a structured NO-OP.

## 2026-07-19 — honesty (ENGINE/diagnostics) — `read_range_window` now carries the same post-restart SPAN-honesty guard its sibling `lead_concentration_window` already has: a full-count but short-span ring honest-nulls instead of asserting a multi-day 24h HIGH off ~30s of data
- Item: roadmap 1.af (the runner-up named in the 2026-07-18 (later²) diagnosis; the last unblocked candidate of the 2026-07-18 bug-hunt cohort — 1.ad and 1.ae are both Robert-gated design/calibration judgments, 1.ae feeds P).
- Diagnosis (weakest reachable seam): suite green (650 passed / 5 ignored — no failing/flaky test to fix first). AWARENESS is the currently-strongest pillar (many recent engine/honesty fixes); the weakest reachable seam is a HONESTY defect on a served self-context read. `read_range_window` (aggregator.rs) publishes the operator's "24h high/low + Position" band and its sibling `lead_concentration_window` (same 24h window, same MIN_SAMPLES=30) already guards `span_secs < MIN_SPAN_SECS` (6h = window/4) with `{available:false, reason:"short_history"}` — the exact guard added after the 2026-07-17 outage exposed the "entrenched · 100% of 24h" post-restart lie. `read_range_window` computed the identical `span_secs` but never gated on it: ~30s after a restart the 1 Hz ring clears the COUNT floor (30 samples) while spanning ~29s, and the served `position:"near-high"` / `pct_rank` then assert a multi-day HIGH that is really seconds of data — the only tell being the un-served `span_secs`. A genuine, reachable, sibling-asymmetric honesty gap on a served surface.
- Change: added `READ_RANGE_MIN_SPAN_SECS = READ_RANGE_WINDOW_SECS / 4` (6h, mirroring `LEAD_CONC_MIN_SPAN_SECS`) and the span guard in `read_range_window` — after the count floor, honest-null `{available:false, reason:"short_history", samples, span_secs}` when the in-window span is under the floor (the `span_secs` computation moved up to the guard, removing the later duplicate). The client `renderReadRange` already renders `available:false` as the honestly-labeled "session high/low" degraded state under the "not-yet-available (cold timeline)" tooltip — so the post-restart read now degrades to a per-tab value LABELLED "session" instead of a bogus multi-day "24h high" rendered under the durable label. No dashboard change (routine-frozen surface, and none needed). Diagnostic only — computed after P is final, never feeds P or any fitted constant; the four calibration anchors carry only fresh events so the guard is a no-op on them.
- Metric moved: served `read_range` honesty on a short-span (post-restart) ring: `available:true` + `position:"near-high"` off ~30s → `available:false` / `reason:"short_history"` until ≥6h span accrues (invariant sibling-lockstep restored). Test count 650 → 652 (+1 lock; net +2 including the split, but only 1 new test added by this run — see proof). Four anchors unchanged (in-band). Build + full suite green.
- Proof: `cargo build --release` green (3m06s). `cargo test --release` → **652 passed / 5 ignored / 0 failed**. New lock `read_range_window_honest_null_on_a_short_post_restart_ring` passes; fails-without-change proven by neutering the threshold to 0 → the 40-sample/39s ring returns `available:true`, test panics at aggregator.rs:3913. The lock also asserts the guard RELEASES (same 40 reads over ~7h span → `available:true`), proving it is span-gated not a blanket refusal. All 6 pre-existing `read_range` tests still green (their oldest ticks span 10h/8h/20h/n-a/6h-boundary/10h, all ≥ the 6h floor).
- Tier: T1 (engine-behavior — corrects a REACHABLE honesty defect on a served self-context read so a full-count-but-short-span ring can no longer claim a multi-day range verdict off seconds of data; a correctness/honesty fix keyed to the model's OWN sibling-diagnostic discipline (the `lead_concentration_window` `short_history` span guard added after the 2026-07-17 outage), restoring lockstep between two reads that share the window/sample-floor contract — not a new light/annotation/surface, board stays at 12, not in the closed blind-thin-stale-… caveat family. Same class the 2026-07-16 window-leak (1.aa) and 2026-07-17-later² floor-held-momentum (1.34) fixes landed as: "diagnostic-served, never feeds P … behavior-changing on a served fallible path, written contract rationale, lock fails-when-reverted." Candid scope note: it never feeds P and reachability needs a service restart followed by a fast operator glance (~lower frequency than a per-tick substring trap); I claim T1 engine-behavior because it changes the COMPUTED output of a served surface, has a written contract rationale — the sibling's own span-honesty comment — and its lock fails-when-stashed, matching the tier's engine-behavior gate.) · Touched: engine-behavior · Lock-fails-without-change: yes (neuter READ_RANGE_MIN_SPAN_SECS→0 → short-span ring returns available:true, panic at aggregator.rs:3913) · Counts: none of Live-sources/Map-layers/Monitors moved (a correctness fix, not a new sight) · consecutive_display_only=0 (engine-behavior, resets the streak) · display_only_in_last_7=1 (the 2026-07-17-later 2.10 still in trailing-7; the 2026-07-16 1.y aged out) · consecutive_noop=0 · noop_in_last_3=0
- Notes / decisions future runs must respect: `read_range_window` and `lead_concentration_window` now BOTH gate on `span_secs < window/4` — keep them in lockstep; a future window/sample-floor change to one must mirror the other. The client `renderReadRange` fallback (dashboard.html:1144) treats `available:false` as "show the tab session extent labelled 'session'" — that path is now REACHED by the short-span case too, which is the intended honest degrade; do not silently relabel it "24h". Remaining open candidates 1.ad (heat-half of the escalation guard) and 1.ae (recency-blind `alliance_invoked` latch, FEEDS P) are BOTH Robert-gated design/calibration judgments — do not take unattended.

---

## 2026-07-18 (later²) — honesty (ENGINE/physical-nuclear) — the apex-Systemic trigger `nuclear_use_in` now declines HISTORICAL-COMMEMORATION framing, so a Hiroshima/Nagasaki anniversary headline carrying "atomic bombing" no longer forces the catastrophe rung (index pegged at 95)
- Item: roadmap 1.36 (new; companion to 1.ab negation + 1.ac's bug-hunt lineage).
- Diagnosis (weakest reachable seam): suite green (648 passed / 5 ignored — no failing/flaky test to fix first). Two fanned-out honesty bug-hunts (aggregator diagnostics / theater-models) surfaced several genuine reachable defects; I took the highest consequence × reachability one. Math-analytic was freshly advanced (1.ac same day); new-source is the signal-hunter's lane; dashboard/board/eyes are operator-frozen; the alliance-coupler recency-latch candidate FEEDS P and is a durable-invocation design call (Robert-gated — parked, not taken unattended); the `read_range_window` span guard was the runner-up. `nuclear_use_in` (src/theater.rs) is the SOLE path to `EscalationRung::Systemic` (docstring: "pegs the headline at the 95 forecast ceiling and floods P(WWIII)"). Its `USE_PHRASES` include `"atomic bombing"` — the phrase that floods the wire every August for the Hiroshima (Aug 6) / Nagasaki (Aug 9) anniversaries. A commemoration headline ("Hiroshima atomic bombing 81st anniversary", "Japan commemorates the atomic bombing", "Nagasaki atomic bombing memorial") carries the use-phrase, is tagged `nuclear_indicator`, is neither interrogative nor negated, and hits no NON_USE token — so it slipped every existing guard (1.ab covered negation/threat/test/allegation, not commemoration) and FALSELY forced the apex Systemic rung. Reachability is seasonal and near-certain: the anniversaries are ~3 weeks out from this run.
- Change (engine-behavior; no P math, no fitted constant, no anchor touched): added the historical-commemoration family {anniversary, anniversaries, commemorate/-s/-d/commemorating, commemoration/-s, commemorative, memorial/-s, remembrance} to `NON_USE_TOKENS`. These are safe whole-word ANYWHERE in the window — like the untrue-report family (hoax/debunked/…) — because a commemorative noun never co-occurs with a fresh detonation report, and `any()` still trips on a real strike's cleaner confirming coverage. DELIBERATELY NOT gated: the bare "N years since / N years ago" ordinal — that construction is SHARED with a real first-in-decades use ("first nuclear weapon used since 1945"), so gating it would risk MUTING a genuine report (a principled recall boundary, documented inline). Docstring updated to name the commemoration guard.
- Metric moved: a Hiroshima/Nagasaki anniversary headline can no longer light the served apex `EscalationRung::Systemic` / peg the headline at 95 (a false-positive on the system's single most consequential trigger, seasonally imminent, is closed). +2 tests (648 → 650 passed). Never feeds P — the four anchors (quiet/Ukraine/current/Cuba) are bit-identical (backtest titles carry no commemorative phrasing; bands all in-band).
- Proof: `cargo build --release` green (2m03s). `cargo clippy --release -p gcrm` — 0 warnings. `cargo test --release` → **650 passed / 0 failed / 5 ignored**; `bands_quiet/ukraine/current_full/cuba` all ok. Lock proven fails-without-change EMPIRICALLY: stashing ONLY the 3 new token lines makes `commemoration_of_a_historical_atomic_bombing_does_not_force_systemic_rung` FAIL (panic at theater.rs:2585 — "Hiroshima atomic bombing 81st anniversary…" forces Systemic); restored → 650 green. Recall guard `a_real_detonation_near_an_anniversary_still_fires_when_a_clean_headline_is_present` proves a clean confirmation co-occurring with a memorial headline still forces Systemic.
- Tier: T1 (engine-behavior — corrects a REACHABLE, seasonally-imminent, high-consequence false-positive on the SOLE apex-Systemic trigger so historical commemoration can no longer masquerade as a fresh nuclear use; it changes WHETHER `nuclear_use_in` fires and thus whether the headline pegs at 95, locked by a fails-when-stashed test with a written contract rationale — the module's own strict "confirmed detonation, not posture/history" discriminator; not a new light/annotation/surface, the board stays at 12. Candid scope note: it is an existing served path, not a new sight, and the effect surfaces only when "atomic bombing" coverage is live; I claim T1 because it is behavior-changing on a served apex path with a stashable lock, matching the tier's engine-behavior gate — the same basis as 1.ab/1.35.) · Touched: engine-behavior · Lock-fails-without-change: yes (stash the 3 token lines → commemoration test panics at theater.rs:2585) · Counts: none of Live-sources/Map-layers/Monitors moved (a correctness fix, not a new sight) · consecutive_display_only=0 (engine-behavior, resets the streak) · display_only_in_last_7=2 (the 2026-07-16 1.y + 2026-07-17-later 2.10 still in trailing-7; this run is engine-behavior) · consecutive_noop=0 · noop_in_last_3=0
- Notes future runs MUST respect: (1) The commemoration guard is INTENTIONALLY scoped to unambiguous commemorative nouns (anniversary/commemoration/memorial/remembrance); the bare "N years since/ago" ordinal is NOT gated on purpose (shared phrasing with a real first-since use) — do NOT add "years"/"since"/"ago" tokens, they would risk muting a genuine report. (2) These tokens live in the GLOBAL whole-word scan (safe anywhere), NOT the scoped before-phrase negator pass — do not move them. (3) This is a served-rung honesty fix on the apex trigger; it never feeds P — the four anchors are untouched. (4) Bug-hunt residue still OPEN: the alliance-coupler recency-latch (`alliance_invoked` un-decayed, no gate/release, FEEDS P via +0.30 coupler — likely Robert-gated, durable-invocation design call) and the `read_range_window` post-restart span guard (served-only, sibling `lead_concentration_window` has it) — see roadmap candidates before acting.

## 2026-07-18 (later) — honesty (ENGINE/diagnostics) — band-coverage now reconstructs each anchor's half-width at FULL 1 Hz resolution (the exact sample set the published band used), not from the 300:1 decimated anchors — so `floored`/`floor_bound_pct`/`mean_hw_pct` stop contradicting the very band they validate
- Item: roadmap 1.ac (candidate filed 2026-07-18 by the aggregator diagnostics bug-hunt; verified still open against current code).
- Diagnosis (weakest reachable seam): suite green (647 passed / 5 ignored — no failing/flaky test to fix first). Math-analytic diagnostics are my lane and 1.ac is an OPEN, cloud-verifiable HONESTY defect on a served self-validation gauge; new-source is the signal-hunter's lane; dashboard/board/eyes are operator-frozen; fitted constants are Robert-gated; 1.ad (the sibling candidate) is explicitly Robert-gated (a design call on the escalation heat half) so I did NOT take it unattended. The routine streak sat at display_only_in_last_7=2 (at the cap), so a substantive engine-behavior run was required — 1.ac fit exactly.
- Change (engine-behavior on a served diagnostic; no P, no fitted constant, no anchor touched): `band_coverage_window` published `floor_bound_pct` (share of bands the ±7pp humility floor set) and `mean_hw_pct` (mean band half-width) reconstructed from a 300:1 STRIDE-DECIMATED series (~72 samples/6h), while the band it VALIDATES — `uncertainty_window` — is built over EVERY in-window read (~21,600/6h). Decimation draws the central-80% (P10–P90) spread from far fewer tail samples, biasing `emp_hw` narrow: a band `uncertainty_window` reported `floored:false` (spread set by measured volatility) could be mislabelled `floor_bound` here, so the sharpness reads contradicted the published band about the identical history. FIX (option a): each anchor's half-width is now rebuilt from the FULL-resolution trailing `band_secs` window (a monotonic `lo_full` left-edge over the full-resolution ascending series → the exact sample set `uncertainty_window` used, minus the confidence-widening term as already documented). The stride-decimated series is retained ONLY as the forward-coverage pair-count autocorrelation control (its stated purpose). Because the fuller per-anchor reconstruction is far heavier, `band_coverage()` is now stride-cached (mirroring `momentum_lead_lag`'s cache) so it never runs the full-resolution scan per 1 Hz broadcast under the shared epoch_store lock.
- Metric moved: the served band-coverage sharpness reads (`floor_bound_pct`/`mean_hw_pct`) now match the published band's own `floored` verdict on the same history instead of biasing floor-bound high / half-width low. +1 test (647 → 648 passed). Never feeds P — the four anchors (quiet/Ukraine/current/Cuba) are bit-identical (backtest bands in-band; this path is a post-P diagnostic and is not exercised by the backtests). `coverage_pct` itself is unchanged in direction (it was already conservative either way).
- Proof: `cargo build --release` green (2m47s). `cargo clippy --release -p gcrm` — 0 warnings. `cargo test --release` → **648 passed / 0 failed / 5 ignored**. Lock proven fails-without-change EMPIRICALLY: reverting the win-gather to the decimated `series` window makes `band_coverage_window_rebuilds_the_half_width_at_full_resolution_not_the_decimated_anchors` FAIL (`floor_bound_pct` 100.0 vs the fix's 0.0, panic at aggregator.rs:3516); restored → 648 green. Existing band_coverage fixtures (samples spaced at exactly 300s → decimation is a no-op) are byte-identical under the fix, so no prior lock moved.
- Tier: T1 (engine-behavior — corrects a served self-validation diagnostic so `floored`/`floor_bound_pct`/`mean_hw_pct` no longer contradict the very band they claim to validate; changes the VALUE of served fields, locked by a fails-when-reverted test with a written contract rationale (the reconstruction must use the same sample set the published band did). Candid scope note: it is a diagnostic (never feeds P) and the magnitude is MEDIUM per the candidate — a bias, not a false alarm; I claim T1 because it is behavior-changing on a served honesty surface with a stashable lock, matching the tier's engine-behavior gate. Chosen because the bug-hunt-surfaced 1.ac was the highest reachable item I could do WELL today: math-analytic is my lane, new-source is cross-lane, dashboard/board/eyes are operator-frozen, fitted constants + 1.ad are Robert-gated, and the streak was at the display-only cap so a substantive run was required.) · Touched: engine-behavior · Lock-fails-without-change: yes (decimated reconstruction → floor_bound_pct 100 vs 0, panic at aggregator.rs:3516) · Counts: none of Live-sources/Map-layers/Monitors moved (a correctness fix, not a new sight) · consecutive_display_only=0 (engine-behavior, resets the streak) · display_only_in_last_7=2 (the 2026-07-16 1.y + 2026-07-17-later 2.10 still in trailing-7; this run is engine-behavior) · consecutive_noop=0 · noop_in_last_3=0
- Notes future runs MUST respect: (1) `band_coverage_window` reconstructs each anchor's HALF-WIDTH from the FULL-resolution trailing window; the stride-decimation is ONLY the forward-coverage pair-count control — do NOT "simplify" the half-width back onto the decimated `series` (that re-admits the narrow-spread bias; the lock pins full-res floor_bound=0 on the sub-anchor-swing fixture). (2) `band_coverage()` is now stride-cached like `momentum_lead_lag` — the cache wraps the PUBLIC entry only; `band_coverage_window` (the tested core) is uncached and deterministic. Do not remove the cache: the full-res reconstruction is too heavy for the 1 Hz broadcast lock. (3) This is a served-diagnostic honesty fix; it never feeds P — the four anchors are untouched. (4) 1.ad remains OPEN and Robert-gated (the escalation-coherence/breadth heat-half question) — resolve with Robert before acting.

## 2026-07-18 (operator session, later) — legibility (DISPLAY) — six-balance: a 6th command-strip metric (Momentum) and a 6th domain-row card (Observation), and the knock-tooltip reach is now pixel-based
- Item: ad-hoc (Robert, 2026-07-18 continued: "even numbers appear balance on mobile devices" — six cells / six cards to mirror the six theaters).
- Changes (display only; NO new scored modality — P and the five model domains untouched):
  1. **Command strip → 6 cells**: added **Momentum** (`couplers.systemic_momentum`, already served) — ⇧/⇩/─ with the theater-chip ±0.25 decisive bar so the two surfaces can't disagree; new `momentum` info-modal states the lead-lag honesty (leading is MEASURED, honest-null until proven).
  2. **Domain row → 6 cards**: added **OBSERVATION** — window-coverage % + gap/watching label + sources/newest-age sub, explicitly labeled "not a force domain" (epistemic: how much to trust the five cards beside it); click opens the Confidence explainer, does NOT filter articles. Grid `--dcols` is `DID.length+1`.
  3. **Knock-tooltip reach fixed**: the nearest-spike matcher was ±8 INDEXES — micro-pixels on a 350k-point history, humanly unhittable (caught in live verification). Now pixel-based (≥12px, ±8-tick floor when zoomed in).
  4. **Finance radar client last-good**: one failed poll no longer blanks the panel to "market feed unavailable" (the map first-miss class); the message shows only when there has never been data.
- Metric moved: +4 tests (647 → 651). Sandbox :8010: eyes green; DOM checks green (6 cells with Momentum populated, 6 cards with Observation populated, momentum modal opens). Live-verified the prior session's drivers in prod first (6 knock entries in today's timeline incl. a +0.5pp politico_eu attribution and an aljazeera-live whisper attribution).
- Tier: operator session (display; board stays 12; no scored modality added — a 6th MODEL domain would be a calibration session with Robert, deliberately not done here) · Lock: `dashboard_top_strip_and_domain_row_balance_to_six` · routine streak counters UNCHANGED.
- Follow-up (5d461b8, same day): the Momentum cell's NEUTRAL state drops the '─ ' prefix — beside '+0.04' it read as a stray minus sign (Robert); decisive ⇧/⇩ arrows unchanged.
- Notes future runs MUST respect: (1) The OBSERVATION card is epistemic display — do NOT convert it into a scored modality or feed it to P. (2) The six-count balance is an operator layout directive; keep strip cells and domain cards at even counts if adding surfaces (which remain operator-gated anyway). (3) The tooltip reach must stay pixel-based — an index window regresses to unhittable at full history.

## 2026-07-18 (operator session) — legibility/awareness (DISPLAY + diagnostics) — Robert-directed: eight architect directives — hoverable "why did it move" knocks on the timeline, model internals collapsed to one toggle, operator-pinned theater strip, streak-gated map feed-health, reversed market palette, readable theater text, calmer ticker, always-spinning brand
- Item: ad-hoc (Robert's Architect's Notes, 2026-07-18, verbatim directives; interactive session, not a routine run).
- Changes (all DISPLAY/diagnostic; P and every fitted constant untouched):
  1. **Timeline movement attribution** (the one engine-adjacent piece, the `seismic_test_consistent` pattern — set AFTER compute, never feeds P): when a tick moves |Δ| ≥ `DRIVER_NOTE_MIN_DELTA` (0.0005 = 0.05pp; measured against prod: 22 such ticks per 6h ≈ ~90 curated knocks/day), the run loop records the tick's top new events (severity-first, "source · title", max 3, 96-char clip), the corroboration count, or the decay/eviction fact onto `snap.tick_drivers` → served top-level (`tick_drivers`) and durably as `TimelineEntry.drivers` (skip-serialized when empty so the 350k ring + JSONL archive stay lean; `append_timeline` now takes the once-built entry so archive and ring cannot diverge). Chart: history spikes SEED from durable driver-carrying entries on load (previously spikes were live-session-only and every reload wiped them), live spikes carry `d.tick_drivers`, spike cap 20→300, and a nearest-x/intersect-false tooltip renders the drivers on hover of any knock (`afterBody`). Locks: `timeline_entry_carries_the_tick_driver_attribution_leanly`, `dashboard_timeline_knocks_explain_their_drivers_on_hover`. Contract doc updated (`tick_drivers` row + the 2026-07-17 observation meta keys that were missing from the table).
  2. **Model internals collapsed**: context-strip + formula + meta-pills wrapped in `#model-internals`, collapsed by default behind one `.mi-toggle` bar; the choice persists (`gcrm-internals-open`); every id and copy stays in the DOM and keeps populating while hidden (sibling tests + the eyes gate keep reading them; the locus honest-null hidden state is an accepted eyes pass). Lock: `dashboard_model_internals_collapse_to_one_toggle`. Aligns with the standing "dashboard rail minimal" directive.
  3. **Theater strip operator pin**: `us_iran` renders FIRST regardless of raw heat (client-side, display only; `topTh`/the Threat-Level cell still read the true heat-sort); strip note now "operator focus · then hottest" with the directive+date in its title. Rationale recorded: NATO–Russia currently reads hotter on posture/alliance weightings; Robert ranks the live shooting war first "as per the reality". NOT a heat/calibration change.
  4. **Map feed-health streak gate** (`osint.rs`): per-feed consecutive-miss streaks (`FEED_MISS_STREAK`); the FIRST missed rebuild is silent (the map still shows last-good) — with ~45 public upstreams a single 429/timeout is routine jitter, and the old first-miss amber kept the strip lit nearly always, so a real outage looked identical to a hiccup (Robert: "hyper-awareness thing"). From `FEED_MISS_SURFACE_STREAK` (2) consecutive misses: ONE line per feed (streak + what is shown instead — the old path pushed the raw error AND the fallback note), raw fetch errors surface only when there is nothing to fall back on, and a long-quiet feed (no error, no recent last-good) is NEVER flagged — quiet is a valid world-state (LAYER_HINTS already explain empty layers). OpenSky's anonymous-credit 429s (the loudest first-miss noise) pass through the same gate. Locks: `miss_note_is_silent_on_the_first_miss_and_names_a_standing_one`, `miss_note_surfaces_a_real_outage_but_never_flags_a_quiet_feed`.
  5. **Reversed market palette** (operator gut call, "until further notice"): price UP = red, DOWN = green in the Finance Radar; arrows stay factual (▲ up / ▼ down) — only the colour carries the reversed valence. GCRM-only — the gmm Markets monitor is field-native and untouched absent a separate directive.
  6. **Theater strip readability**: chip text 8px/7px → 10px/8.5px, taller heat bar, more padding (operator: "text is too small to read easily").
  7. **Ticker** scroll 80s → 120s.
  8. **Brand coin always spins**: removed the blanket reduce-motion `animation:none` (this box's browser/OS preference had frozen it — the raithe-server-only symptom) + a motion watchdog that falls back to a compositor-independent 2D rotation if the 3D flip renders frozen (e.g. graphics acceleration off).
- Metric moved: +7 tests (640 → 647 passed on the combined tree incl. the 1.35 routine commit). Verified on a sandboxed :8010 instance: eyes green; DOM checks green (pin first-chip = US/Israel–Iran, collapse/expand/hidden-population, 120s ticker, coin `brand-a-spin/running`, ▲-red/▼-green market rows); the driver gate correctly stayed silent on a settled store (max tick |Δ| 0.00009 < gate) and was tuned against prod's measured tick-delta distribution, not guessed.
- Tier: operator session (dashboard-surface changes are Robert-authorized in-session — the 2026-07-09 surface freeze binds ROUTINES, not the operator; no new I&W light, the board stays 12) · Touched: display + diagnostics (+ run-loop attribution that never feeds P) · Lock-fails-without-change: yes (per-change locks above) · routine streak counters UNCHANGED (not a routine run).
- Notes future runs MUST respect: (1) The theater-strip pin and the reversed market palette are OPERATOR directives — do NOT "fix" them back to hottest-first / conventional colours; both are labeled in-UI and revert only on Robert's word. (2) `DRIVER_NOTE_MIN_DELTA` is a display threshold tuned to prod's measured tick-delta distribution — retune only against fresh measurements, never to "show more knocks". (3) The map health strip is DELIBERATELY silent on a first miss and on long-quiet feeds — do not restore first-miss surfacing (that is the crying-wolf failure it ends); a feed dark ≥2 rebuilds still surfaces, and the watchdog roster floor is unchanged. (4) Model internals ship COLLAPSED; do not move the content out of the DOM to "simplify" — hidden population is what keeps the sibling locks and the eyes gate honest. (5) The brand coin is exempt from prefers-reduced-motion by operator requirement; every other animation keeps honoring it.

## 2026-07-18 — honesty (ENGINE/physical-nuclear) — the aftershock discriminator stopped reading an EMPTY single-source (USGS-only) response as a CONFIRMED absence of aftershocks — the strongest physical nuclear indicator no longer lights off USGS simply lacking catalog data for a remote test-site region
- Item: roadmap 1.35 (new). Lane-3 ENGINE/CODE-QUALITY honesty repair on the highest-consequence PHYSICAL path.
- Diagnosis (weakest reachable seam): suite green (640 passed / 5 ignored — no failing/flaky test to fix first). Math-analytic was freshly advanced 2026-07-17 (1.33 slope); new-source is the signal-hunter's lane; the I&W board + caveat family + dashboard surfaces + eyes checks are operator-frozen; fitted constants are Robert-gated; the streak sat at display_only_in_last_7=2 (at the cap), so this run HAD to be substantive engine-behavior. A fanned-out three-front bug-hunt (detector / theater+bayesian / aggregator) surfaced three genuine reachable defects; I took the highest-consequence one. `check_aftershocks` (detector.rs) hardcodes a USGS-only 2h re-query and set `AftershockAbsent` (→ served `seismic_test_consistent` I&W indicator + the +0.20 confidence absence bonus) whenever `aftershock_count == 0` — WITHOUT distinguishing "USGS confirmed no aftershocks" from "USGS returned an empty response (no coverage)". The detector's own FDSN registry notes USGS ComCat completeness for small events in the Arctic / Central-Asian test sites (Novaya Zemlya, Lop Nur, Semipalatinsk) is poor and GFZ/EMSC cover those better — so a natural M≥4.5 quake there, whose real aftershock sequence GFZ/EMSC record, can hit an empty USGS response and FALSELY light the system's strongest physical nuclear indicator. Absence of coverage read as evidence of absence.
- Change (engine-behavior; no P, no fitted constant, no anchor touched): capture the TOTAL features USGS returned (coverage proof) alongside the aftershock count (that set minus the mainshock). New pure `aftershock_verdict(returned, count)` → `Sequence` (count ≥ 2, clear) / `Inconclusive` (returned == 0 → no coverage, leave the alert UNTOUCHED — no `AftershockAbsent`, no bonus) / `Absent` (returned ≥ 1 with zero aftershocks → USGS demonstrably saw the region → confirmed absence) / `Ambiguous` (count == 1). `check_aftershocks` routes through it; `apply_aftershock_verdict` extracted for the per-alert mutation so the whole decision+consequence is offline-testable. The +0.20 absence bonus is now only awarded when the region was actually covered (Inconclusive keeps `aftershock_checked == false`).
- Metric moved: a natural quake in a USGS-dark test-site region can no longer light `seismic_test_consistent` or claim the absence bonus off an empty response (a false-positive on the strongest physical nuclear indicator is closed). +2 tests (640 → 642 passed). Never feeds P — the four anchors are bit-identical (backtest bands quiet/Ukraine/current/Cuba in-band; the seismic path is not exercised by the backtests).
- Proof: `cargo build --release` green (1m54s). `cargo clippy --release -p gcrm` — 0 warnings. `cargo test --release` → **642 passed / 0 failed / 5 ignored** (backtest bands held). Lock proven fails-without-change EMPIRICALLY: neutering the `returned == 0` guard (`sed` → `returned == usize::MAX`, so empty → Absent) makes BOTH `aftershock_verdict_reads_an_empty_usgs_response_as_inconclusive_not_confirmed_absence` (panic at detector.rs:1237) and `empty_usgs_response_does_not_light_the_seismic_test_consistent_indicator` (panic at detector.rs:1258) FAIL; restored → 642 green.
- Tier: T1 (engine-behavior — corrects a REACHABLE, high-consequence false-positive on the strongest PHYSICAL nuclear indicator so an absence of source coverage can no longer masquerade as a confirmed absence of aftershocks; a correctness/honesty fix keyed to the detector's own multi-network coverage asymmetry — 5-network detection but 1-network discrimination — not a new light/annotation/surface, the board stays at 12. It changes WHETHER `seismic_test_consistent` lights and whether the +0.20 bonus applies. Candid scope note: `is_test_consistent` never feeds P, and reachability needs a natural quake in a USGS-dark test-site region — lower-frequency than an everyday substring trap, but the blast radius is a false read on the system's most alarming physical indicator; I claim T1 because it is behavior-changing on a served fallible path, has a written contract rationale (the module's own "no aftershock sequence → explosion" discriminator + the FDSN registry's coverage notes), and its lock fails-when-reverted). Chosen because the bug-hunt surfaced this genuine defect as the highest-consequence of three, math-analytic was freshly advanced, new-source is cross-lane, dashboard/board/eyes are operator-frozen, fitted constants are Robert-gated, and the streak was at the display-only cap so a substantive run was required. · Touched: engine-behavior · Lock-fails-without-change: yes (neuter `returned == 0` → both tests panic at detector.rs:1237/1258) · Counts: none of Live-sources/Map-layers/Monitors moved (a correctness fix, not a new sight) · consecutive_display_only=0 (engine-behavior, resets the streak) · display_only_in_last_7=2 (the 2026-07-16 1.y + 2026-07-17-later 2.10, both still in trailing-7; this run is engine-behavior) · consecutive_noop=0 · noop_in_last_3=0
- Notes future runs MUST respect: (1) The aftershock discriminator distinguishes coverage-absent (empty response → `Inconclusive`, no verdict) from aftershock-absent (covered + zero → `Absent`). Do NOT "simplify" back to `aftershock_count == 0 → AftershockAbsent` — that re-admits the false-positive (the lock pins verdict(0,0) → Inconclusive). (2) `Inconclusive` leaves `aftershock_checked == false` on purpose so `compute_confidence` withholds the +0.20 bonus and the one-shot 2h check does not claim a verdict it never earned. (3) The USGS-only query is unchanged — the fix is about the SEMANTICS of an empty response, not adding a source (map/feed sources are the signal-hunter's lane). (4) This is a served-indicator/confidence fix; it never feeds P — the four anchors are untouched. (5) TWO prospecting candidates were filed from the same bug-hunt: 1.ac (`band_coverage_window` decimated-vs-full-resolution band reconstruction — served `floor_bound_pct` biased high; MEDIUM, has a clean gate) and 1.ad (escalation reads omit the `heat ≥ STABLE_HEAT_CEILING` half of the sibling guard — likely Robert-gated design call, resolve before acting).

## 2026-07-17 — honesty (ENGINE/confidence + diagnostics) — OPERATOR SESSION (Robert-directed, interactive): confidence now measures whether the pipe was WATCHING the window, not merely how much it holds — a post-outage read stops claiming "Confidence: 100%" across an unobserved hole; day-share ring verdicts stop rendering off minutes of post-restart ring
- Item: ad-hoc (operator-directed after the 2026-07-15→17 host outage: cooler failure, box dark ~48h across three crash-boots. On reboot the live read showed P 76.9% CRITICAL, "Confidence: 100%", every modality "100% conf", `lead_concentration` "entrenched · 100% of 24h" off a ~15-minute ring — while only 334 events carried `published_at` within the 72h window (histogram: dense pre-outage edge, a ~30h middle hole with two zero buckets, a 25-min post-reboot trickle) and only 13 distinct sources had reported in 6h. `events_in_window` (4702) and `sources_active` (108) use the ~10-day live-recency gate, so every blend term saturated on stale-but-warm evidence. Robert: "GCRM should reflect reality not a fiction.")
- Diagnosis (pillar-1 HONESTY): `estimate_confidence` blends amount-of-evidence terms (tier 0.5 + log-sat volume 0.3 + breadth 0.2); the only staleness guard was `is_data_blind` (events == 0 under the ~10-day gate), a binary that cannot trip until ~10 days of total darkness. Nothing measured TEMPORAL coverage: a store that comes back warm from a multi-day outage is neither blind nor thin, yet the middle of its window went unwatched while `read_held_by_floor` (correctly, per the declared error posture) held P up — extrapolation-through-a-gap masquerading as a fully-measured read. Sibling defect: `lead_concentration_window` guarded samples by COUNT only (30 ticks ≈ 30 s at 1 Hz), so a freshly restarted ring rendered a day-share verdict off minutes, and the dashboard hardcoded "% of 24h".
- Change (engine-behavior on served display fields; P untouched, no fitted constant): new pure `bayesian::observation_coverage(events, now, window_hours)` — the 72h window split into 3h buckets, each saturating at OBSERVATION_BUCKET_SATURATION=5 events ("was the pipe observing this interval" — deliberately far below the healthy ~45+/bucket rate so a quiet news regime never reads as an outage), × a freshness term (1.0 within a 3h grace of the newest live event, then halving every OBSERVATION_STALE_HALF_LIFE_HOURS=12h of silence). The factor multiplies `estimate_confidence` AND every per-modality "% conf" (avg_domain_conf extracted from undiscounted values first, so the discount applies exactly once), floored at the offline floors. New snapshot + meta fields `window_coverage` / `newest_event_age_secs` / `observation_factor` / `observation_gap` (`has_observation_gap`: coverage < 0.90 OR newest age > grace); the CRITICAL alert line carries the gap note beside the confidence it qualifies; header watchdog + I&W summary render an OBSERVATION GAP caveat (a text qualifier in the existing summary — NO new light, the board stays at 12); conf info-modal + methodology explain the factor via server-templated OBS_* constants (the anti-drift render test now locks 10 placeholders). The uncertainty band inherits honest widening through its existing (1 − confidence) term. `lead_concentration_window` now requires span ≥ LEAD_CONC_MIN_SPAN_SECS (window/4 = 6h; honest-null `short_history` below it) and serves `span_secs`; the dashboard quotes the day-share against the actual span ("% of last Nh"), never a hardcoded 24h.
- Metric moved: over the live post-outage store (offline replay of the 18:37 state) the headline would have read ~73% confidence with an explicit gap caveat instead of 100%. Verified A/B on a sandboxed :8010 instance over the same event archives: after its fast keyword-path backfill genuinely re-observed the window (GDELT archive + RSS retention; every 3h bucket ≥8 events) it read confidence 0.992 = coverage 0.992, gap=false — proving the metric recovers exactly when the window is genuinely re-observed, and discounts exactly while it is not (prod's GPU-capped enricher drains its backlog over hours, so prod will show the caveat until the hole actually refills). +7 tests (626 → 633 passed).
- Proof: `cargo test` **633 passed / 0 failed / 5 ignored**. Locks fail-without-change: removing the factor multiply fails `compute_discounts_confidence_and_modality_cells_across_an_observation_gap`; removing the span guard fails `lead_concentration_window_honest_null_on_a_short_post_restart_ring`; healthy continuous operation is locked byte-identical (factor exactly 1.0) by `observation_coverage_is_full_for_a_continuously_observed_window`. Eyes gate green against the sandboxed new binary (locus honest-null renders as the expected hidden state; all panels intact, both viewports).
- Tier: T1 (engine-behavior on served honesty fields; OPERATOR SESSION — the dashboard-surface edits (header caveat branch, I&W summary qualifier, modal/methodology prose) are Robert-authorized in this interactive session; the 2026-07-09 operator freeze on new surfaces/lights binds ROUTINES and is not breached — no new I&W light, board stays 12) · Touched: engine-behavior + display · Lock-fails-without-change: yes (three locks above) · Counts: none of Live-sources/Map-layers/Monitors moved · consecutive_display_only / display_only_in_last_7: UNCHANGED for routine accounting (not a routine run — the next routine inherits the 2026-07-14 (later²) streak state: consecutive_display_only=0, display_only_in_last_7=1, and 1.28 ages out one slot sooner)
- Notes future runs MUST respect: (1) The observation factor is DISPLAY-ONLY — do NOT feed it into P, the persistence floor, or theater heat; the declared error posture (hold an active war through news gaps) is intact BY DESIGN, and the factor exists precisely so that holding ≠ claiming certainty. (2) Do NOT re-chase "confidence reads low after a restart/outage" as a bug — that is the honest state; it recovers as the window genuinely refills (coverage) while the newest signal stays fresh. (3) OBSERVATION_BUCKET_SATURATION=5 is deliberately far below healthy per-bucket volume so quiet ≠ unobserved; do not raise it to "make confidence stabler", and do not lower the 0.90 caveat floor to hide the caveat. (4) `lead_concentration` honest-nulls (`short_history`) for ~6h after any restart — expected, not a regression; `alert_dwell` already carried `capped` and `trend_6h` already carried span-honest copy (verified in-session — do not re-add). (5) `events_in_window`/`sources_active` KEEP the ~10-day live gate on purpose (they feed `is_data_blind`/`is_thinly_sourced` and the watchdog roster floor); the 72h-window truth lives in `window_coverage`/`newest_event_age_secs`. (6) The three OBS display thresholds (bucket sat 5, grace 3h, stale half-life 12h, caveat floor 0.90) are operator-set display constants, not fitted calibration — still Robert-gated: do not blind-tweak.

## 2026-07-14 (later²) — honesty (ENGINE/regime) — the regime-multiplier gate stopped accepting a sub-0.00005 input that rounds to 0.0 and collapses the regime product (the "model destruction" its own bound documents preventing)

## 2026-07-17 (later²) — honesty (ENGINE/escalation-reads) — the escalation-COHERENCE and escalation-BREADTH gauges stopped counting a FLOOR-HELD theater's STALE momentum as live escalation — the same live-evidence guard the sibling `systemic_momentum` gauge already applies, so the two reads can no longer contradict it about the same theater
- Item: roadmap 1.34 (new). Lane-1 ENGINE/CODE-QUALITY honesty repair on the awareness gauges.
- Diagnosis (weakest reachable seam): suite green (631 passed / 5 ignored — no failing/flaky test). A fanned-out bug-hunt of the high-consequence engine/analytics paths (theater/models/bayesian/backtest/aggregator diagnostics) surfaced ONE genuine, live, reachable honesty defect: the escalation-coherence (1.19) and escalation-breadth (1.20) reads — added AFTER `systemic_momentum` — omitted its floor-held guard. Math-analytic diagnostics are deeply mined and were freshly advanced the same day (1.33 slope); new-source is the signal-hunter's lane; fitted constants are Robert-gated; the I&W board + caveat family are operator-frozen; and the last run was display-only AT the 2-of-7 cap (so this run had to be substantive). A P-adjacent honesty correctness fix keyed to the model's own sibling-diagnostic discipline was the highest-value item doable well today.
- Change (one coherent engine fix; no P, no computed calibration value, no fitted constant, no anchor touched): added `!t.held_by_floor` to BOTH escalation filters in `bayesian::compute` — the momentum-leader for `escalation_coherence` (bayesian.rs:1134) and the fronts list for `escalation_breadth` (bayesian.rs:1164). `escalation_momentum` is computed unconditionally from a theater's (possibly aged) events, so a floor-held theater (displayed heat = a remembered war-state carried through a news gap) carried a STALE escalatory momentum. Without the guard, a sustained war whose coverage lulls ~a day would be named the "decisively escalating" front (coherence) AND phantom-counted as a synchronized front (breadth) — a PRESENT-TENSE claim minted from memory — while `systemic_momentum` (which excludes floor-held, theater.rs:883) correctly reads the LIVE direction: two gauges from the same number contradicting each other. The fix is the exact live-evidence guard the sibling gauge documents and uses.
- Metric moved: an honesty inconsistency on two served AWARENESS gauges is closed (they now agree with `systemic_momentum` on which theaters carry live news-flow). +1 Rust lock test (631 → 632 passed). No P / no anchor moved: the reads are diagnostic-only (never feed `l_sys`/P), so the four anchors are bit-identical (quiet/Ukraine/current 60/Cuba in-band; backtest bands + calibration evidence unchanged).
- Proof: `cargo build --release` green. `cargo test --release` → **632 passed / 0 failed / 5 ignored** (backtest bands quiet/Ukraine/current/Cuba held). Lock proven fails-without EMPIRICALLY: stashing the `!held_by_floor` guard on both filters (sed) + rebuild → `escalation_reads_exclude_a_floor_held_theaters_stale_momentum` FAILS precisely — "a floor-held theater's stale escalatory memory must NOT mint a coherence read; got momentum_theater_id=\"us_iran\"" (panic at bayesian.rs:2489); restored → 632 green.
- Tier: T1 (engine-behavior — corrects a REACHABLE honesty defect on two served fallible gauges so their present-tense escalation claim can no longer be minted from stale floor-held memory; a correctness/honesty fix keyed to the model's own sibling-diagnostic discipline — the `systemic_momentum` `!held_by_floor` guard — not a new light/annotation/surface, and not in the closed I&W-board / blind-thin-stale-… caveat family. Same class the 2026-07-16 window-leak fix (1.aa) landed as: "diagnostic-served, never feeds P … behavior-changing on a served fallible path, written contract rationale, lock fails-when-reverted." Candid scope note: these two reads never feed P, and reachability needs a sustained-war coverage lull (~a day) — a slightly-lighter T1 than the high-frequency substring fixes; I claim T1 engine-behavior because it changes the COMPUTED output of two served gauges, has a written contract rationale (the sibling gauge's own honesty comment), and its lock fails-when-stashed). Chosen because the bug-hunt surfaced this genuine defect, the math-analytic lane was freshly advanced the same day, new-source is cross-lane, dashboard/board/eyes are operator-frozen, fitted constants are Robert-gated, the last run was display-only at the 2-of-7 cap, and the suite was green. · Touched: engine-behavior · Lock-fails-without-change: yes (stash both `!held_by_floor` guards → coherence names the floor-held us_iran, test panics at bayesian.rs:2489) · Counts: none of Live-sources/Map-layers/Monitors moved (a correctness fix, not a new sight) · consecutive_display_only=0 (engine-behavior, resets the streak) · display_only_in_last_7=2 (the 2026-07-16 1.y + 2026-07-17-later 2.10; this run is engine-behavior, so the cap is cleared) · consecutive_noop=0 · noop_in_last_3=0
- Notes future runs MUST respect: (1) The guard is `!held_by_floor` on `escalation_momentum > +0.30` — the SAME live-evidence discipline as `systemic_momentum` (theater.rs:883). Both escalation filters must keep it in lockstep with the sibling gauge; if you ever add a THIRD escalation-momentum consumer, it needs the same guard or it re-opens this contradiction. (2) The strict `>` (not `>=`) mirror of the de-escalation gate is unchanged and still locked by `escalation_decisive_bar_is_a_strict_mirror_of_the_de_escalation_gate` — the new guard composes with it (`!held_by_floor && m > +0.30`). (3) The over-suppression half of the lock proves a FRESH decisively-escalating theater STILL counts — do not "simplify" the guard into a blanket exclusion that would also drop live escalation. (4) This is diagnostic/awareness only — never feeds P; the four anchors are untouched.

## 2026-07-17 (later) — legibility/honesty (VISUAL-ANALYTIC) — the served honesty pages are verified COMPLETE: no page ships a raw `{{TOKEN}}` template placeholder (build-time self-check + comprehensive Rust test), and the eyes gate now SEES the /methodology whitepaper it was blind to
- Item: roadmap 2.10 (new). Lane-2 VISUAL-ANALYTIC + a pillar-1 render-completeness invariant on the honesty surfaces.
- Diagnosis (weakest reachable seam): suite green (630 passed / 5 ignored — no failing/flaky test to fix first). A fresh bug-hunt of the high-consequence engine paths (coupler/heat/great-power/calibration) found NO defect — `great_power_label` is exact-`match` (no substring trap), the alliance-tier leak was closed in 1.23, and every `{{...}}` placeholder is currently substituted. The math-analytic lane was FRESHLY advanced this same day (1.33 slope); new-source is the signal-hunter's lane; fitted-constant VALUES are Robert-gated; the I&W board + caveat family are operator-frozen. That left pillar-2 render VERIFICATION as the weakest reachable seam: the `/methodology` whitepaper — which carries the calibration evidence, the anchor table, and the alert bands, made MORE load-bearing this week by 1.y (bands from live config) and 1.33 (scale slope) — is 100% UNVERIFIED: the eyes gate only ever loaded `/risk`. And the served pages are built by a hand-maintained `.replace()` chain whose per-token tests each check only their OWN token, so a NEW `{{FOO}}` added without its substitution would ship raw template syntax (`{{ALERT_CRITICAL}}`, a broken honesty claim) to the operator, unseen — a template-drift hole nothing closes.
- Change (render-completeness verification; no P, no computed value, no fitted constant, no anchor touched): (a) `unsubstituted_placeholders(&str) -> Vec<String>` — a pure byte-scan that returns any surviving `{{[A-Z0-9_]+}}` token (the token run is ASCII so the slice is always a char boundary; `{{ALERT_30D}}` carries a digit, so the charset is `[A-Z0-9_]`, not `[A-Z_]`). (b) A prod-safe startup self-check in `ServerState::new` that `error!`-logs any leftover on BOTH rendered pages — never panics (a cosmetic render defect must not take the service down) and leaves the raw token shipping (it screams broken rather than hiding), but ops is now ALERTED instead of the defect going unseen. (c) A comprehensive Rust test `served_pages_carry_no_unsubstituted_template_placeholder`: asserts the fully-rendered dashboard AND methodology carry ZERO `{{TOKEN}}`, that a live value genuinely rendered on each (Brier on the whitepaper, the elevation-threshold constant on the dashboard — guards a vacuous pass), that the detector BITES an injected `{{ZZ_TEST_TOKEN}}`, and that a non-default base_path renders clean. (d) `smoke.mjs` checks #1c (the /risk raw served HTML carries no `{{...}}`) and #1d (fetch `/methodology`: 200, no `{{...}}`, the calibration-evidence block "Brier" and a real alert-band `%` both rendered) — the eyes gate now SEES the honesty page it was blind to (STAGED — the cloud sandbox has no running service; the local deploy runs the browser gate).
- Metric moved: the render-completeness gap on the served honesty pages is closed at build time (the one test that FAILS the moment a placeholder is added without its substitution), and the eyes gate now covers `/methodology` (was blind). +1 Rust test (630 → 631 passed). No P / no anchor touched: this is render-verification only — the four anchors are bit-identical (quiet 2.62 / Ukraine 43.24 / current 60.01 / Cuba 84.30; Brier 0.00092 / RMSE 3.04pp / in-band 4/4 unchanged).
- Proof: `cargo build --release` green. `cargo test --release` → **631 passed / 0 failed / 5 ignored** (backtest bands quiet/Ukraine/current/Cuba in-band, held). `node --check deploy/eyes/smoke.mjs` OK. Lock proven fails-without-change EMPIRICALLY: injecting `<!-- {{ZZ_DRIFT_PROBE}} -->` into `methodology.html` (no matching `.replace()`) makes the test FAIL precisely — `methodology shipped unsubstituted placeholder(s): ["{{ZZ_DRIFT_PROBE}}"]` (panic at server.rs:1701); reverted → 631 green.
- Tier: T2 (VISUAL-ANALYTIC — a new deploy/eyes rung that makes the gate SEE the `/methodology` honesty page it never loaded, PLUS a build-time render-completeness invariant on the served pages; precedent: 2.7 "the eyes gate can SEE the I&W board" and 2.9 "the eyes gate JUDGES the viewports" were graded creditable gate-extensions, and this closes a genuinely-unverified surface + a template-drift honesty hole the per-token tests structurally cannot catch. NOT annotation — it adds a NEW thing the system can SEE about itself, not a caveat mirrored to an Nth surface; NOT in the closed I&W-board / blind-thin-stale-… caveat family). Chosen because the engine bug-hunt came up empty, the math-analytic lane was freshly advanced the same day, new-source is cross-lane, fitted constants are Robert-gated, and the suite was green — so the highest-value item I could do WELL was closing the verification gap on the most honesty-load-bearing served page. · Touched: display-only (a deploy-gate/JS change + a Rust guard test + a prod-safe log; no engine/P behavior — display-only for the streak, the "lock" is the render-completeness invariant, proven fails-without by the injected-token test above) · Lock-fails-without-change: yes (injected `{{ZZ_DRIFT_PROBE}}` → test FAILS at server.rs:1701) · Counts: none of Live-sources/Map-layers/Monitors moved — a gate/verification rung · consecutive_display_only=1 · display_only_in_last_7=2 (this + 2026-07-16 1.y; the 2026-07-16-later² nuclear and 2026-07-17 slope are engine/calibration — AT the 2-of-7 cap, so the NEXT run MUST be T1/T2-substantive, not a 3rd display-only) · consecutive_noop=0 · noop_in_last_3=0
- Notes future runs MUST respect: (1) The placeholder charset is `[A-Z0-9_]+` (NOT `[A-Z_]+`) — `{{ALERT_30D}}` carries a digit; a narrower charset would miss it. (2) The startup self-check is PROD-SAFE by design — it `error!`-logs and never panics; do NOT "harden" it into a startup panic (a cosmetic render defect must not take the service down, and the raw token already screams broken). (3) `unsubstituted_placeholders` scans the FULLY-RENDERED page (post-substitution); every `{{` in both raw templates is a real placeholder (verified 27=27 / 30=30) and `calibration_evidence_html()` emits none, so a rendered page is `{{`-free. (4) The smoke.mjs /methodology + placeholder checks are STAGED — the local deploy promotes them; do NOT report DONE from the cloud. (5) This is DISPLAY/verification only — never feeds P; the four anchors are untouched.

## 2026-07-17 — honesty (MATH-ANALYTIC/calibration) — the calibration evidence now reports the SCALE (does the model over- or under-REACT up the ladder?), not just the LEVEL bias — decomposing the +2.29pp miss into level + scale
- Item: roadmap 1.33 (the SCALE sequel to 1.26's level-bias diagnostic; standing math-analytic lane #1 — "deepen what the model KNOWS about itself").
- Diagnosis (weakest reachable seam): suite green (629 passed / 5 ignored — no failing/flaky test to fix first). New-source is the signal-hunter's lane; the I&W board + caveat family + dashboard surfaces + eyes checks are operator-frozen (2026-07-09); fitted-constant VALUES are Robert-gated; 1.x needs a WEEK of labeled video↔wire pairs (not doable well on 11 samples). The recent streak is a long run of honesty substring/negation ENGINE fixes — the top standing lane (MATH-ANALYTIC: calibration diagnostics that deepen what the model knows about itself) was least-recently advanced (last was 1.26, 2026-07-12). 1.26 added the calibration-in-the-large (mean signed error +2.29pp, unanimous 4/4) — HOW HIGH the model sits — but a model can sit right on average yet MIS-SCALE: over-react toward the apex (predictions too extreme) or under-react (too compressed), a distinct, orthogonal failure the mean error AND the squared Brier/RMSE cannot express. That was the sharpest genuinely-NEW-and-TRUE read I could add well today.
- Change (calibration DIAGNOSTIC; no P, no served operational number, no fitted constant, no anchor touched): added `calibration_slope(pairs) -> Option<(slope, intercept)>` — the standard risk-prediction calibration regression (Cox 1958; Steyerberg, *Clinical Prediction Models*; Van Calster et al. 2016, "A calibration hierarchy"): ordinary least-squares of the anchored centres (OUTCOME) on the model P (PREDICTION), `centre ≈ intercept + slope·P`, ideal slope 1.00 / intercept 0. Slope < 1 → predictions too EXTREME (over-dispersed across the ladder); > 1 → too COMPRESSED. Returns `None` on < 2 pairs or zero prediction spread (no divide-by-zero). Surfaced on `CalibrationEvidence` (`calib_slope`/`calib_intercept`), in the `calibration_evidence_report` `--nocapture` readout ("scale:" line), and as a "Scale calibration" clause on the EXISTING methodology fragment (extends 1.26's directional paragraph — NOT a new surface).
- Metric moved: the calibration evidence now states the SCALE alongside the LEVEL. Live read: **slope 0.968 / intercept −0.79pp → slightly OVER-dispersed (too extreme)**, but |slope−1| = 0.032 < 0.05, so the +2.29pp miss is almost entirely a uniform LEVEL shift, not a distortion of the ladder's shape — a genuinely reassuring TRUE statement about the ranking. +1 test (629 → 630 passed). Diagnostic only — never feeds P; the four anchors are bit-identical (quiet 2.62 / Ukraine 43.24 / current 60.01 / Cuba 84.30; Brier 0.00092 / RMSE 3.04pp / signed-bias +2.29pp / in-band 4/4 unchanged).
- Proof: `cargo build --release` green (1m43s). `cargo test --release` → **630 passed / 0 failed / 5 ignored** (backtest bands quiet/Ukraine/current/Cuba in-band, held). `cargo clippy --release -p gcrm` — 0 warnings. Calibration report prints `scale: calib-slope=0.968 intercept=-0.79pp (over-dispersed / too extreme)`. Lock proven fails-without-change: `calibration_evidence_reports_the_scale_slope_not_just_the_level_bias` pins the OLS identity (independent recompute), the live slope in (0.90, 1.00), orthogonality to bias (an unbiased-but-compressed set → slope > 1), the too-extreme→0.5 / too-compressed→2.0 / perfect→1.0/0 discrimination, zero-spread & <2-pair → None, and the html stating the slope + "over-dispersed". Neutering `calibration_slope` to a constant 1.0 flips the live-slope assert (1.0 ∉ (0.90,1.00)) and drops the "over-dispersed" html clause → the test FAILS.
- Tier: T1 (calibration diagnostic — the top standing math-analytic lane: it deepens what the model KNOWS about its own calibration by decomposing the miss into level + scale, a genuinely new-and-true read the existing three numbers cannot express, keyed to the model's own anchors, not a new light/gauge-from-new-input/source/surface. Meets the calibration-lane gate: written literature rationale (Cox/Steyerberg/Van Calster) + a test that FAILS when stashed + the four anchors stay in-band. Candid scope note: this is computed FROM the four anchors, not "beyond" them over epoch history (1.12 band-coverage was the "beyond" one) — a slightly-lighter T1 than a new epoch-history diagnostic; I claim T1 because it is a substantive new computed decomposition with a fails-without lock and a literature contract, exactly the class the 1.26 signed-bias sibling landed as, and the mission prizes the genuinely-new-and-true read over the merely-easy-to-prove). Chosen because 1.x needs a week of labeled pairs, new-source is cross-lane, dashboard/board/eyes are operator-frozen, fitted constants are Robert-gated, the suite was green, and the math-analytic lane was least-recently advanced. · Touched: calibration · Lock-fails-without-change: yes (neuter slope→1.0 → live-slope assert fails + html "over-dispersed" clause drops) · Counts: none of Live-sources/Map-layers/Monitors moved (a diagnostic, not a new sight) · consecutive_display_only=0 (calibration diagnostic, substantive — resets/holds the streak) · display_only_in_last_7=1 (only the 2026-07-16-later 1.y; still within trailing 7) · consecutive_noop=0 · noop_in_last_3=0
- Notes future runs MUST respect: (1) `calibration_slope` regresses the OUTCOME (anchor centre) on the PREDICTION (model P) — the risk-prediction standard where slope < 1 = predictions TOO EXTREME. Do NOT flip the regression direction (centre-on-P, not P-on-centre) or the interpretation inverts. (2) It is a DIAGNOSTIC — never feeds P, touches no fitted constant; the four anchors stay bit-identical. Do NOT "use" the slope to retune anything (that would be Robert-gated calibration tuning, and fitting to 4 points is statistically weak — the diagnostic is honest evidence, not a target). (3) The "Scale calibration" clause extends the EXISTING {{CALIBRATION_EVIDENCE}} fragment — it is not a new operator surface (consistent with 1.26). (4) Remaining open [candidate]: 1.x (video↔wire merge-threshold fit — needs a WEEK of labeled pairs, do NOT hand-tune on 11 samples).

## 2026-07-16 (later²) — honesty (ENGINE/nuclear-use) — the apex nuclear-USE trigger stopped forcing the systemic (index-pegged-at-95, P-flooding) rung on a report of the ABSENCE of a strike
- Item: roadmap 1.ab (ad-hoc; a contract-vs-code gap in the sole apex trigger, surfaced by a fresh bug-hunt of `nuclear_use_in` — the only path to the Systemic rung).
- Diagnosis (pillar-1 HONESTY, the run's weakest reachable seam): suite green (627 passed / 5 ignored — no failing/flaky test to fix first). 1.x is the only open [candidate] and needs a WEEK of labeled video↔wire pairs (not doable well on 11 samples). New-source is the signal-hunter's lane; the I&W board + caveat family + dashboard surfaces + eyes checks are operator-frozen (2026-07-09); fitted-constant VALUES are Robert-gated; the prior run flagged the display-only cap at consecutive=1 and asked the NEXT run to prefer engine-behavior. So I hunted the highest-consequence engine path fresh: `nuclear_use_in` (theater.rs) is the ONLY trigger for `EscalationRung::Systemic`, which pegs the headline at `FORECAST_INDEX_CEILING = 95` and floods P(WWIII). It guards threat/test/allegation/drill/advocacy/interrogative framing (1.7-class hardening) but had NO negation guard — so a headline carrying a use-phrase (`nuclear strike`/`nuclear detonation`…) + `nuclear_indicator` that REPORTED THE ABSENCE of a strike ("No nuclear strike occurred", "Investigators find no evidence of a nuclear strike", "report debunked as a hoax", "claim was fabricated") FALSELY forced the catastrophe rung. This is the single most consequential false-positive reachable in the system — one self-contained wire headline pegging P(WWIII) at max.
- Change (engine-behavior; no P math, no fitted constant, no served number, no anchor touched): after the existing interrogative + global non-use scans, require at least one use-phrase occurrence that is NOT locally negated. Two mechanisms: (a) scoped preceding negators `{no,not,never,nor}` within `NEG_LOOKBACK = 4` tokens immediately BEFORE the phrase's first word — before-only, because a trailing "no survivors"/"no doubt" trails a REAL detonation, so a global scan would kill recall; (b) an unambiguous report-is-untrue family `{hoax,debunked,disproven,disproved,fabricated,fabrication,untrue,misinformation,disinformation,nonexistent}` added to the global `NON_USE_TOKENS` (these never co-occur with a genuine confirmation; "false" deliberately omitted — "false flag" can accompany a real attribution dispute). Also switched the DECIDING phrase match from substring (`t.contains(p)`) to word-sequence (`windows` equality), dropping the phrase's own substring traps; the cheap `contains` stays as a fast-reject. Recall preserved by `any()` over the window + before-only scoping (locked by the trailing-negator confirmation test).
- Metric moved: a negated / untrue nuclear-absence report can no longer force the Systemic rung / peg the systemic index at 95 / flood P(WWIII). +2 tests (627 → 629 passed). The four anchors are bit-identical: backtest event titles are "Analog backtest event headline text" (no use-phrase), so `nuclear_use_in` never fires in the backtests regardless of `nuclear_indicator` — quiet 2.62 / Ukraine 43.24 / current 60.01 / Cuba 84.30, Brier 0.00092 / in-band 4/4 unchanged.
- Proof: `cargo build --release` green. `cargo test --release` → **629 passed / 0 failed / 5 ignored** (backtest bands quiet/Ukraine/current/Cuba in-band, held). `cargo clippy --release -p gcrm` — 0 warnings. Lock proven fails-without-change: stashing ONLY the non-negated-phrase requirement (keeping the two new tests + the untrue-family additions) makes `negated_nuclear_report_does_not_force_systemic_rung` FAIL (panic at theater.rs:2533 — "No nuclear strike: tensions ease after border standoff" forces `Systemic`); restored → 629 green.
- Tier: T1 (engine-behavior — corrects a REACHABLE, high-consequence behavior on the apex P-pegging trigger to add the negation guard its threat/test/allegation siblings imply; a correctness/honesty fix on a P-flooding served path, keyed to the model's own use-vs-non-use taxonomy, not a new light/annotation/surface — the board stays at 12. It changes WHETHER the Systemic rung fires, which pegs the index at 95 and floods P — a directly-P-consequential fix. Candid scope note: reachability requires a nuclear_indicator-tagged headline that literally reports the absence of a strike, lower-frequency than the everyday substring fixes, but the BLAST RADIUS is maximal — a single such headline pegs P(WWIII); I claim T1 because it is behavior-changing on the highest-consequence fallible path, has a written contract rationale (the function's own strict-use docstring + its threat/test/allegation guards), and its lock fails-when-stashed). Chosen because 1.x needs a week of labeled pairs, new-source is cross-lane, dashboard/board/eyes are operator-frozen, fitted constants are Robert-gated, and the suite was green — so the highest-value item I could do WELL was this verified-open honesty defect on the apex trigger. · Touched: engine-behavior · Lock-fails-without-change: yes (stash the negation requirement → "No nuclear strike…" forces Systemic, test panics at theater.rs:2533) · Counts: none of Live-sources/Map-layers/Monitors moved (a correctness fix, not a new sight) · consecutive_display_only=0 (engine-behavior, resets the streak) · display_only_in_last_7=1 (only the 2026-07-16-later 1.y; 1.z/1.aa/this are engine-behavior) · consecutive_noop=0 · noop_in_last_3=0
- Notes future runs MUST respect: (1) `nuclear_use_in` now requires a NON-negated use-phrase occurrence — do NOT "clean up" back to the bare `!non_use_framing` return (re-admits the absence-report false positive; the lock pins "No nuclear strike…" → not Systemic). (2) The bare negators `{no,not,never,nor}` are scoped BEFORE-only (≤4 tokens) — do NOT move them into the global `NON_USE_TOKENS` list (a trailing "no survivors" would then mute a REAL detonation; the recall test `negation_after_the_phrase_still_fires_systemic_rung` pins this). (3) The untrue family (hoax/debunked/fabricated/…) IS global whole-word — safe because it never describes a real confirmation; "false" was deliberately left out (false-flag ambiguity). (4) This is P-CONSEQUENTIAL (gates the apex rung) but the four anchors are unaffected because backtest titles carry no use-phrase — a future recalibration must not start feeding use-phrase titles into the backtest without re-checking this guard. (5) Remaining open [candidate]: 1.x (video↔wire merge-threshold fit — needs a WEEK of labeled pairs).

## 2026-07-16 (later) — honesty (SERVED/methodology) — the whitepaper's alert bands now render from the LIVE config, not `AlertSettings::default()` (a page that promised it "cannot disagree with the classification" could silently state the wrong bands the moment an operator tuned `alerts:`)
- Item: roadmap 1.y (surfaced 2026-07-14 by a served-path bug-hunt; explicitly pre-classified display-only, "take on a run with display-only headroom").
- Diagnosis (pillar-1 HONESTY, the run's weakest reachable seam): suite green (626 passed / 5 ignored — no failing/flaky test to fix first). The `t > now` window seam (1.aa) is fully closed — I re-grep-verified all seven EpochStore window loops (`trend`/`uncertainty`/`read_range`/`band_coverage`/`alert_dwell`/`lead_concentration`/`momentum_lead_lag`) now carry the guard. New-source is the signal-hunter's lane; the I&W board + caveat family + dashboard surfaces + eyes checks are operator-frozen (2026-07-09 directive); fitted-constant VALUES are Robert-gated. Of the remaining open [candidate]s, 1.x needs a WEEK of labeled video↔wire pairs (not doable well today — do NOT hand-tune on 11 samples), leaving 1.y: a REAL, verified-open served-page honesty defect with a clean falsifiable gate, and the display-only cap had headroom (trailing-7 carried only the 2026-07-13 1.28, and it ages out with this entry). `ServerState::new` (server.rs:66) filled the served `/methodology` page's Elevated/Critical/30-day band figures from `crate::models::AlertSettings::default()` (2.5/8.0/1.0%), NOT the loaded `settings.alerts` — while the adjacent comment AND the served prose both promise the bands are "read from the engine's live `AlertSettings` … so the prose, the colour, and the chart cannot disagree with the classification." The engine CLASSIFIES with `settings.alerts` and the live snapshot serves those thresholds, so the dashboard is live-correct; only the whitepaper was pinned to defaults — latent today only because committed `settings.yml == defaults`, but the moment an operator tunes `alerts:` the whitepaper silently states the WRONG bands for the whole run, contradicting the live hero readout + timeline reference lines.
- Change (display/awareness honesty; no P, no computed value, no fitted constant touched): threaded `&settings.alerts` into `ServerState::new` (a 3rd `alerts: &AlertSettings` param — the value is already in scope at the main.rs call site, next to where it is handed to the aggregator) and rendered the three `{{ALERT_*}}` placeholders from it instead of `::default()`. This makes the whitepaper a genuine single-source-of-truth on the live config (the anti-drift guarantee its own comment/prose copies from the neighbouring compile-time constants), matching how the dashboard already reads the live thresholds. Updated the 11 test call sites to the 3-arg form (passing `&AlertSettings::default()` where the band value is irrelevant).
- Metric moved: a tuned `alerts:` config can no longer make the served methodology page state the wrong bands (the whitepaper now honours its own "cannot disagree with the classification" contract). +1 test (626 → 627 passed). No P / no anchor touched: the render is startup HTML, never feeds `l_sys`/P; the four anchors are bit-identical (quiet 2.62 / Ukraine 43.24 / current 60.01 / Cuba 84.30; Brier 0.00092 / in-band 4/4 unchanged).
- Proof: `cargo build --release` green (1m44s). `cargo test --release` → **627 passed / 0 failed / 5 ignored**. `cargo clippy --release -p gcrm` — 0 warnings. Lock proven fails-without-change: re-pinning the render to `let alerts = &AlertSettings::default();` (shadowing the param, keeping the new test) makes `methodology_renders_alert_bands_from_the_live_config_not_defaults` FAIL (panic at server.rs:1622 — the page shows the default `8.0%`, not the tuned `5.0%`/`12.0%`/`3.0%`); restored → 627 green.
- Tier: T3 (a served-page honesty correctness fix — the whitepaper bands now follow the live config the classifier uses, honouring the page's own anti-drift contract; NOT a new light/gauge/source/surface, and not in the closed blind/thin/stale/… family. I claim T3 rather than T1 engine-behavior because it changes only a rendered display string conditioned on config — no computed value or P moves — exactly as the roadmap item pre-classified it, though its lock does fail-when-reverted). Chosen because 1.x needs a week of labeled pairs (not doable well today), new-source is cross-lane, dashboard/board/eyes are operator-frozen, fitted constants are Robert-gated, the suite was green, and the display-only cap had headroom — so the highest-value item I could do well was this verified-open served-page honesty defect. · Touched: display-only (changes a served display string conditioned on config; no computed value or P moves) · Lock-fails-without-change: yes (re-pin render to `::default()` → page shows default `8.0%`, test panics at server.rs:1622) · Counts: none of Live-sources/Map-layers/Monitors moved (a correctness fix, not a new sight) · consecutive_display_only=1 · display_only_in_last_7=1 (only this run; the 2026-07-13 1.28 ages out with this entry — 1.29/1.30/1.31/1.32/1.z/1.aa are all engine-behavior) · consecutive_noop=0 · noop_in_last_3=0
- Notes future runs MUST respect: (1) The methodology bands render from the LIVE `settings.alerts` threaded through `ServerState::new`, NOT `AlertSettings::default()` — do NOT "clean up" back to `::default()` inside `new` (re-admits the silent-wrong-bands drift; the lock pins that a `critical=0.12` config shows `12.0%` and NOT the default `8.0%`). (2) The dashboard hero/timeline were already live-correct (they read the live snapshot at runtime); only the startup-baked whitepaper had the defect. (3) This is DISPLAY/awareness only — never feeds P; the four anchors are untouched. (4) Remaining open [candidate]: 1.x (video↔wire merge-threshold fit — needs a WEEK of labeled pairs, do NOT hand-tune on 11 samples). The display-only cap is now consecutive=1 / 1-of-7 — the NEXT run should prefer an engine-behavior/new-source item.

## 2026-07-16 — honesty (ENGINE/windows) — a future-dated ring entry no longer leaks into the served 6h-trend / uncertainty band / read-range (three window loops now honor the CLOSED [now−window, now] interval their docs + four siblings promise)
- Item: roadmap 1.aa (engine-behavior; surfaced 2026-07-14 by a served-path bug-hunt, parked for a run with headroom).
- Diagnosis (pillar-1 HONESTY, the run's weakest reachable seam): suite green (626 passed / 5 ignored — no failing/flaky test to fix first); new-source is the signal-hunter's lane; the I&W board + caveat family + dashboard surfaces + eyes checks are operator-frozen (2026-07-09 directive); fitted-constant VALUES are Robert-gated. Of the open [candidate] items, 1.aa is a P-adjacent SERVED honesty defect (three window loops disagree with their own doc AND with the four sibling diagnostics on the same tick), strictly higher value than 1.y (display-only methodology-band pin) and comparable-but-broader than the 1.x threshold-fit (needs a week of labeled pairs — not doable well today). Picked 1.aa.
- Change: `EpochStore::trend_window` (aggregator.rs:646), `uncertainty_window` (:716), and `read_range_window` (:792) guarded only the lower bound (`if t < cutoff { break; }`). Their docs promise the CLOSED interval `[now − window_secs, now]` and the four siblings that claim the same discipline — `band_coverage_window` (:927), `alert_dwell_window` (:1106), `lead_concentration_window` (:1183), `momentum_lead_lag_window` (:1290) — all carry `if t > now { continue; }`. So a future-dated tick (NTP step-back, or ticks persisted while the clock ran ahead then reloaded via `load_epoch`) was COUNTED by trend/uncertainty/read_range but EXCLUDED by the others: an extreme future read could set the range high, widen the uncertainty band, and inflate the trend sample count, making the served `trend_6h`/`uncertainty`/`read_range` disagree with the doc and with the sibling diagnostics. Added the identical `if t > now { continue; }` guard to all three loops (`continue`, not `break` — a future tick sits at the newest end, so it must be skipped without truncating the still-valid older window).
- Metric moved: served window reads now match their `[now−window, now]` contract on a clock-skewed tick (invariant repaired); test count 625→626 (+1 lock).
- Proof: `cargo build --release` green (3m05s). `cargo test --release` → 626 passed / 5 ignored / 0 failed (backtest bands quiet/Ukraine/current/Cuba in-band, held). Lock `future_dated_ring_entry_is_excluded_from_trend_uncertainty_and_read_range` PASSES with the guard; reverting the guard (all 3 loops) → FAILS at aggregator.rs:3730 (future 0.95 → read-range hi 0.55→0.95, pct_rank 100→87.5, position near-high→upper, samples 7→8).
- Tier: T1 (engine-behavior — corrects a SERVED value: `trend_6h`/`uncertainty`/`read_range` now honor the closed-interval contract in a reachable clock-skew case, keyed to the model's own sibling-diagnostic discipline, not a new light/annotation/surface — the board stays at 12. Candid scope note: these three reads are DIAGNOSTIC-served, never feed P, and reachability needs clock skew / future-dated persistence — a slightly-lighter T1 than the high-frequency substring fixes; I claim T1 engine-behavior because it is behavior-changing on a served fallible path, has a written contract rationale (its own doc + four in-repo siblings), and its lock fails-when-reverted). Chosen because it is a P-adjacent served honesty fix strictly higher-value than the two remaining doable candidates (1.y display-only, 1.x needs a week of labeled pairs), new-source is cross-lane, dashboard/board/eyes are operator-frozen, fitted constants are Robert-gated, and the suite was green. · Touched: engine-behavior · Lock-fails-without-change: yes (revert all 3 guards → future 0.95 changes the read-range, test panics at aggregator.rs:3730) · Counts: none of Live-sources/Map-layers/Monitors moved (a correctness fix, not a new sight) · consecutive_display_only=0 (engine-behavior, resets the streak) · display_only_in_last_7=1 (only the 2026-07-13 1.28; the 1.29/1.30/1.31/1.32/1.z/this runs are engine-behavior, 1.26 has aged out) · consecutive_noop=0 · noop_in_last_3=0
- Notes / decisions future runs must respect: 1.aa is now CLOSED. The `if t > now { continue; }` guard is the standard EpochStore window discipline — any NEW window loop must carry it. Remaining open [candidate]s: 1.x (video↔wire merge-threshold fit — needs a WEEK of labeled pairs, do NOT hand-tune on 11 samples), 1.y (methodology-band pin to `AlertSettings::default()` — display-only, take on a run with display-only headroom).
- Item: roadmap 1.z (engine-behavior; parked by the 1.32 run's notes as a served + P-feeding defect surfaced by a served-path bug-hunt — take it on a future run).
- Diagnosis (pillar-1 HONESTY, the run's weakest reachable seam): the display-only cap allowed a display-only run this turn (trailing-7 carried 2: 1.28 + 1.26, and 1.26 ages out with this entry), but a P-feeding engine-behavior fix is strictly higher value than the two parked alternatives (1.y methodology bands = display-only, 1.aa window guards = served-but-never-feeds-P). New-source is the signal-hunter's lane; new dashboard surfaces + eyes checks + I&W lights are operator-frozen (2026-07-09 directive); fitted-constant VALUES are Robert-gated; the suite was green (624 passed, no failing/flaky test to fix first). `set_regime_multiplier` (api.rs:344) gates the sanity bounds on the RAW `body.multiplier` (`<= 0.0 || > 10.0`) but the value actually stored/served/fed-forward is `(body.multiplier * 1e4).round() / 1e4`. Any raw input in `(0.0, 0.00005)` — e.g. `0.00004` — passes the `> 0.0` gate yet rounds to exactly `0.0`. Stored as 0.0, it makes `regime_product` (a `.product()` over active factors) collapse to 0.0 regardless of every other active multiplier, and `guardrail_from_regime(0.0)` then feeds the systemic amplifier — precisely the "accidental model destruction" the bound's own comment documents preventing. The served `product`/`multiplier` also read 0.0, outside the documented `(0.0, 10.0]`. Reachability is contrived (an operator entering a sub-0.0001 multiplier behind the op key), which is why it was parked, not taken first — but it is a self-contradicting safety gate on a P-feeding served path, the sharpest pillar-1 defect reachable today.
- Change (engine-behavior; no fitted constant, no anchor touched): extracted `validated_multiplier(raw) -> Option<f64>` — rounds to the stored 1e-4 precision FIRST, then gates the ROUNDED value against `(0.0, 10.0]` (one source of truth for the gate and the stored value, so they can never disagree again) — and routed the handler through it (`None` → the same 400). A sub-0.00005 input now returns 400 instead of storing 0.0; the smallest surviving input `0.00005` rounds to `0.0001` and is accepted, served strictly > 0 as the bound requires. Side effect (intentional, harmless): a raw input in `(10.0, 10.00005]` now rounds to `10.0` and is ACCEPTED rather than rejected — gating the rounded value is the correct discipline (10.0 is in-range); `10.1` still rounds to `10.1` and is rejected. Rewrote the three bare-inequality bounds tests to exercise `validated_multiplier` (the real gate) instead of re-asserting `m <= 0.0`.
- Metric moved: a sub-0.00005 operator multiplier can no longer round to 0.0 and collapse `regime_product`/`guardrail_from_regime`/the systemic amplifier (a correctness/honesty fix on a P-feeding served path). +1 test (624 → 625 passed). The four anchors are bit-identical: the gate is operator-input-only and the backtests set regime factors directly (never post to this handler), so `cargo test` in-suite incl. backtest is green and calibration evidence (Brier 0.00092 / in-band 4/4) is unchanged.
- Proof: `cargo build --release` clean (2m54s). `cargo test --release` **625 passed / 0 failed / 5 ignored**. `cargo clippy --release -p gcrm` — 0 warnings. Lock proven fails-without-change: reverting ONLY the gate to the raw value (`if raw <= 0.0 || raw > 10.0 { None } else { Some(round(raw)) }`, keeping the helper signature + test) makes `multiplier_that_rounds_to_zero_is_rejected_not_stored_as_zero` FAIL (panic at api.rs:980 — `validated_multiplier(0.00004)` returns `Some(0.0)` vs asserted `None`); restored → 625 green.
- Tier: T1 (engine-behavior — guards a REACHABLE P-feeding served path so a documented safety bound can no longer be defeated by the round-after-gate mismatch; a correctness/honesty fix keyed to the model's own stored precision, not a new light/annotation/surface — the board stays at 12. Candid scope note: reachability is contrived — operator input behind the op key — so it is a slightly-lighter T1 than the high-frequency substring fixes; I claim T1 engine-behavior because it is behavior-changing on a P-feeding fallible path, has a written contract rationale (its own bound comment), and its lock fails-when-reverted). Chosen because it is a strictly higher-value P-feeding fix than the two other parked candidates (1.y display-only, 1.aa never-feeds-P), new-source is cross-lane, dashboard/board/eyes are operator-frozen, fitted constants are Robert-gated, and the suite was green. · Touched: engine-behavior · Lock-fails-without-change: yes (revert-gate-to-raw → 0.00004 stored as 0.0, test panics at api.rs:980) · Counts: none of Live-sources/Map-layers/Monitors moved (a correctness fix, not a new sight) · consecutive_display_only=0 (engine-behavior, resets the streak) · display_only_in_last_7=1 (only 1.28; 1.26 ages out with this entry; 1.27/1.29/1.30/1.31/1.32/this are engine-behavior) · consecutive_noop=0 · noop_in_last_3=0
- Notes future runs MUST respect: (1) The regime multiplier gate lives in `validated_multiplier` (rounds THEN gates); do NOT "clean up" back to gating `body.multiplier` before rounding — that re-admits the round-to-0.0 collapse (the lock pins `0.00004 → None`, `0.00005 → Some(0.0001)`). (2) The gate now accepts a raw input in `(10.0, 10.00005]` (rounds to 10.0) — this is intentional, gating the stored value; `10.1` still rejects. (3) This IS on a P-feeding path but the four anchors are unaffected because they never post to this handler — a future recalibration must not route regime input through the raw value to "hit" an anchor. (4) TWO parked candidates remain from the 1.32 bug-hunt: 1.y (methodology bands render from `AlertSettings::default()` not the live config — display-only, take on a display-only-headroom run) and 1.aa (`trend_window`/`uncertainty_window`/`read_range_window` omit the `t > now` guard their doc + four sibling diagnostics enforce — engine-behavior, served but never feeds P, take on a future run).

## 2026-07-14 (later) — honesty (ENGINE/dispatch) — the LLM-dispatch gate stopped substring-matching person/org names it documents as whole-word (`putin`⊂disputing, `trump`⊂trumpeted, `hamas`⊂bahamas)
- Item: roadmap 1.32 (ad-hoc; a contract-vs-code contradiction surfaced by a fresh bug-hunt of the less-recently-audited engine files — nlp_sidecar/processor/models/video).
- Diagnosis (pillar-1 HONESTY, the run's weakest reachable seam): the display-only cap FORBADE a 3rd display-only this run — my trailing-7 already carries 2 (1.28 multi_theater floor + 1.26 signed-bias), so candidate 1.y (methodology bands, explicitly display-only) was out. New-source is the signal-hunter's lane; new dashboard surfaces + eyes checks + I&W lights are operator-frozen (2026-07-09 directive); fitted-constant VALUES are Robert-gated; the suite was green (624 passed, no failing/flaky test to fix first). A fresh two-front engine bug-hunt found `has_geopolitical_trigger` (nlp_sidecar.rs, the gate deciding whether an article the keyword pass rejected is worth an LLM dispatch — and, via `video::condense_transcript`, which transcript sentences are kept in the 6000-char signal budget) DOCUMENTS "the short tokens ... match whole-word only, mirroring processor::BOUNDARY_ACTOR_PATS ... person/org names ... have no adjective/derived forms" — yet `putin`/`trump`/`hamas` sat in the plain-substring `ACTORS` list. So `trump`⊂trumpeted, `hamas`⊂bahamas, `putin`⊂disputing/computing each fired the gate on ordinary non-geopolitical headlines: a phantom dispatch into the cap=2 (GTX-1080 VRAM) GPU pool that DELAYS a real enrichment, and a false-positive sentence that DISPLACES genuine signal from the video transcript budget. This is the exact substring→word-boundary honesty class the audit hardened on the WHERE/actor side (1.7/1.8/1.21/1.22/1.29/1.30) but left standing on the dispatch gate.
- Change (engine-behavior; no P, no fitted constant, no served number touched): moved exactly `putin`/`trump`/`hamas` from the substring `ACTORS` list to the whole-word `ACTORS_WORD` list (matched via `processor::contains_word`), mirroring `BOUNDARY_ACTOR_PATS`. Kept `zelensky`/`houthi` substring on purpose — they carry suffix forms ("Zelenskyy", "Houthis") that whole-word would DROP, so substring is the correct RECALL choice there (a dispatch gate must not lose a real signal). Net: strictly removes the three documented mid-word false positives with ZERO recall loss (real Trump/Putin/Hamas mentions still fire, incl. across a hyphen/possessive boundary). Corrected the two doc comments to state the actual per-token discipline.
- Metric moved: the three named-actor carrier words (trumpeted/bahamas/disputing-computing) no longer phantom-fire the LLM-dispatch gate or the video signal-sentence selector. No new #[test] function (STRENGTHENED the two existing `geopolitical_trigger_*` tests — no floor move); count holds at 624 passed / 5 ignored. Never touches P or a served number: the gate is upstream of enrichment; the four anchors are bit-identical (the backtests carry no such titles; `cargo test` in-suite incl. backtest green; calibration evidence Brier 0.00092 / in-band 4/4 unchanged).
- Proof: `cargo build --release` clean (1m47s). `cargo test --release` **624 passed / 0 failed / 5 ignored**. `cargo clippy --release -p gcrm` — 0 warnings. Lock proven fails-without-change: reverting ONLY the list move (putin/trump/hamas back to substring `ACTORS`, keeping the strengthened tests) makes `geopolitical_trigger_short_tokens_do_not_fire_inside_ordinary_words` FAIL (panic at nlp_sidecar.rs:569 — "'Officials trumpeted the new stadium deal' must not trigger the LLM dispatch gate"); restored → 624 green.
- Tier: T1 (engine-behavior — corrects a REACHABLE, high-frequency dispatch/selection behavior to match the gate's own documented contract + the in-repo `BOUNDARY_ACTOR_PATS` precedent; a correctness/honesty fix keyed to the model's actor taxonomy, not a new light/annotation/surface — the board stays at 12. Candid scope note: this changes the dispatch/enrichment PIPELINE — GPU-pool efficiency + which video sentences reach NLP/LLM (indirect P via video-sourced events) — NOT a directly-served number, so it is a slightly-lighter T1 than 1.29–1.31; I claim T1 engine-behavior because it is behavior-changing on a fallible path, has a written contract/precedent rationale, and its lock fails-when-stashed). Chosen because the display-only cap forbade a 3rd display-only, new-source is cross-lane, dashboard/board/eyes are operator-frozen, fitted constants are Robert-gated, and the suite was green — so the highest lane I could do WELL was a fresh engine-behavior honesty fix. · Touched: engine-behavior · Lock-fails-without-change: yes (revert-list-move → 'trumpeted' phantom-fires the gate, test panics at nlp_sidecar.rs:569) · Counts: none of Live-sources/Map-layers/Monitors moved (a correctness fix, not a new sight) · consecutive_display_only=0 (engine-behavior, resets the streak) · display_only_in_last_7=2 (1.28 + 1.26; 1.27/1.29/1.30/1.31/this are engine-behavior) · consecutive_noop=0 · noop_in_last_3=0
- Notes future runs MUST respect: (1) `putin`/`trump`/`hamas` are WHOLE-WORD in `ACTORS_WORD` (mirroring `BOUNDARY_ACTOR_PATS`) — do NOT "clean up" back into the substring `ACTORS` list (re-admits trumpeted/bahamas/computing). (2) `zelensky`/`houthi` STAY substring — whole-word would drop "Zelenskyy"/"Houthis"; the lock pins both directions (carriers don't fire, suffix forms do). (3) This is the dispatch GATE, not a served value — it never feeds P directly; its impact is GPU-pool efficiency + video-sentence signal fidelity. (4) TWO distinct served-path defects were surfaced this run and PARKED as roadmap candidates 1.z + 1.aa (both engine-behavior, take on a future run): `set_regime_multiplier` gates the raw value but stores the rounded one (a sub-0.0001 input rounds to 0.0 and collapses the P-feeding regime product — the "model destruction" its own bound documents preventing; contrived operator-input reachability); and `trend_window`/`uncertainty_window`/`read_range_window` omit the `t > now` guard their doc + four sibling diagnostics enforce (a future-dated ring entry leaks into the served trend/uncertainty/read-range under clock skew).
