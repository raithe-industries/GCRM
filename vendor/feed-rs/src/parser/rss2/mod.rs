...
fn handle_item<R: BufRead>(parser: &Parser, element: Element<R>) -> ParseFeedResult<Option<Entry>> {
    ...
    for child in element.children() {
        ...
        match child.ns_and_tag() {
            ...
            (NS::RSS, "description") => entry.summary = util::handle_text(child), // VENDOR PATCH (GCRM): handle_encoded re-parses entity-encoded markup and mangles it ("&lt;p&gt;x" → "px"); plain text capture restores feed-rs 1.x semantics — the consumer's sanitizer owns cleanup
